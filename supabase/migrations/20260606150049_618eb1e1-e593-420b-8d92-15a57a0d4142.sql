...
BEGIN
  ...
  -- Lógica de filtragem agressiva:
  -- 1. Unificamos marcações do mesmo tipo em janelas de 60 minutos.
  -- 2. Se o colaborador bater "Entrada" 3 vezes de manhã, pegamos apenas a primeira.
  ...
  FOR r IN
      ...
      SELECT 
        ...
        CASE 
          WHEN LAG(hora_marcacao) OVER (PARTITION BY tipo_marcacao ORDER BY hora_marcacao) IS NULL 
               OR (hora_marcacao - LAG(hora_marcacao) OVER (PARTITION BY tipo_marcacao ORDER BY hora_marcacao)) > interval '60 minutes'
          THEN 1 ELSE 0 
        END as novo_evento
    ...
    marcacoes_unicas AS (
      -- Pegamos a PRIMEIRA de cada tipo em cada janela (evento)
  ...
  -- Se tiver EXATAMENTE 4 marcações (ou 2), forçamos Regular/Atraso se a paridade estiver ok
  -- Isso resolve casos onde o usuário errou o 'tipo' mas os pares existem
