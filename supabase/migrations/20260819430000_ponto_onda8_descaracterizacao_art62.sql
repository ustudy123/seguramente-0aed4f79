-- ============================================================================
-- ONDA 8 (parte 2) — Controle de fato descaracteriza a dispensa do art. 62
-- PONTO-375
--
-- A dispensa do art. 62 (parte 1) cai na Justiça quando há CONTROLE DE FATO: um
-- vínculo marcado como dispensado que, na prática, acumula marcações reais. Aí a
-- exclusão do art. 62 é descaracterizada e as horas extras do período inteiro
-- voltam. Faltava quem visse isso e avisasse RH/Jurídico a tempo.
--
-- O QUE FAZ (aditivo): ponto_detectar_descaracterizacao_art62 cruza o
-- enquadramento (dispensado_ponto, parte 1) com as marcações reais recentes —
-- dispensado com marcações recorrentes é sinal de descaracterizacao — e gera um
-- alerta crítico por colaborador. Depende do enquadramento do PONTO-373.
--
-- GARANTIAS: só leitura das marcações + gravação de alerta. Não altera o motor de
-- saldo, o espelho, o fechamento nem o enquadramento. Aditivo e idempotente
-- (um alerta por colaborador/dia de varredura).
-- ============================================================================
