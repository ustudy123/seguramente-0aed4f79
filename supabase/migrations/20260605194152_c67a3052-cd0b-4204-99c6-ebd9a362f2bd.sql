CREATE OR REPLACE FUNCTION public.superadmin_list_all_empresas()
 RETURNS TABLE(empresa_id uuid, razao_social text, nome_fantasia text, cnpj text, ativo boolean, empresa_created_at timestamp with time zone, tenant_id uuid, tenant_nome text, tenant_slug text, total_empresas_tenant bigint, is_principal boolean, tenant_owner_email text)
 LANGUAGE sql
...
AS $function$
  WITH base AS (
    SELECT
      ...
      -- A empresa principal é aquela com tipo_unidade = 'matriz'. 
      -- Se houver inconsistência (múltiplas matrizes ou nenhuma), usamos a data de criação como desempate.
      ROW_NUMBER() OVER (
        PARTITION BY e.tenant_id 
        ORDER BY 
          CASE WHEN e.tipo_unidade = 'matriz' THEN 0 ELSE 1 END ASC, 
          e.created_at ASC, 
          e.id ASC
      ) AS rn
