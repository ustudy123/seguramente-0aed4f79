...
BEGIN
  ...
  -- Sem sabado trabalhado: a equalizacao cai no ULTIMO SABADO do mes. Resolver
  -- isto ANTES do laco evita a linha duplicada (a mesma data saia uma vez pelo
  -- laco e outra por um bloco final).
  ...
  LOOP
    ...
    IF NOT v_prot_credito AND v_eq_data IS NOT NULL AND r.data = v_eq_data THEN
      v_eq_teve_linha := true;
    ...
    IF v_ent_esc IS NOT NULL AND v_sai_esc IS NOT NULL
      ...
      v_ent_cons := CASE
        WHEN abs(floor(EXTRACT(EPOCH FROM r.entrada)/60)::int - floor(EXTRACT(EPOCH FROM v_ent_esc)/60)::int) <= COALESCE(v_tol_bat, 10)
          THEN floor(EXTRACT(EPOCH FROM v_ent_esc)/60)::int
        ELSE floor(EXTRACT(EPOCH FROM r.entrada)/60)::int
      END;

      v_sai_cons := CASE
        WHEN abs(floor(EXTRACT(EPOCH FROM r.saida)/60)::int - floor(EXTRACT(EPOCH FROM v_sai_esc)/60)::int) <= COALESCE(v_tol_bat, 10)
