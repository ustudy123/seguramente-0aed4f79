...
BEGIN
  ...
  -- Ordenamos por created_at DESC para garantir que ajustes manuais ou correções recentes (que têm created_at mais novo) prevaleçam
  SELECT (SELECT hora_marcacao FROM public.ponto_marcacoes WHERE tenant_id = NEW.tenant_id AND colaborador_cpf = NEW.colaborador_cpf AND data_marcacao = NEW.data_marcacao AND tipo_marcacao = 'entrada' ORDER BY created_at DESC, id DESC LIMIT 1) INTO v_entrada;
  SELECT (SELECT hora_marcacao FROM public.ponto_marcacoes WHERE tenant_id = NEW.tenant_id AND colaborador_cpf = NEW.colaborador_cpf AND data_marcacao = NEW.data_marcacao AND tipo_marcacao = 'saida_almoco' ORDER BY created_at DESC, id DESC LIMIT 1) INTO v_saida_almoco;
  SELECT (SELECT hora_marcacao FROM public.ponto_marcacoes WHERE tenant_id = NEW.tenant_id AND colaborador_cpf = NEW.colaborador_cpf AND data_marcacao = NEW.data_marcacao AND tipo_marcacao = 'retorno_almoco' ORDER BY created_at DESC, id DESC LIMIT 1) INTO v_retorno_almoco;
  SELECT (SELECT hora_marcacao FROM public.ponto_marcacoes WHERE tenant_id = NEW.tenant_id AND colaborador_cpf = NEW.colaborador_cpf AND data_marcacao = NEW.data_marcacao AND tipo_marcacao = 'saida' ORDER BY created_at DESC, id DESC LIMIT 1) INTO v_saida;

  -- Verificar se existe ajuste pendente no sistema
  SELECT EXISTS (
