-- ============================================================================
-- ONDA 2 (parte 3) — Detecção de marcações uniformes (espelho "britânico")
-- PONTO-377
--
-- Um mês de batidas cravadas no mesmo minuto (08:00 / 12:00 / 13:00 / 17:00
-- todos os dias) hoje passa sem aviso. A Súmula 338, III, do TST considera esses
-- registros INVÁLIDOS como prova e inverte a presunção a favor do empregado. O
-- gerador de alertas atual só conhece falta e atraso.
--
-- O QUE FAZ (aditivo)
--   ponto_verificar_marcacoes_uniformes: por colaborador na competência, mede o
--   desvio-padrão dos horários de entrada e de saída ao longo dos dias
--   trabalhados. Desvio quase nulo por muitos dias = espelho uniforme
--   ("britânico"). Devolve os colaboradores com o diagnóstico.
--   ponto_marcacoes_uniformes_monitorar: roda a verificação e alerta o RH,
--   idempotente por competência.
--
-- Somente leitura sobre ponto_diario; não altera nada do que já existe.
-- ============================================================================
