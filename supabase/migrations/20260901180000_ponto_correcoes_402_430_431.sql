-- =====================================================================
-- PONTO — três correções apontadas pela bateria (402, 430, 431)
--
-- São os três primeiros itens do bloco que falha TANTO na homologação
-- QUANTO no projeto — ou seja, lacunas de verdade, não distância de
-- entrega. Escolhidos por serem baratos e de risco alto.
--
-- (402) FERIADO NÃO TRABALHADO ESTAVA VIRANDO FALTA
--   A consolidação do dia conhece férias, atestado e afastamento — e
--   trata cada um como abono. Feriado, não: o dia de feriado sem
--   marcação caía no cálculo comum e virava falta, descontando salário e
--   derrubando o DSR de quem não devia nada (Lei 605/1949, art. 1º: o
--   feriado é repouso REMUNERADO). Um único feriado espalhava o erro
--   pelo quadro inteiro.
--   Agora, antes do cálculo, o dia sem marcação que for feriado da
--   unidade do colaborador é gravado como dia neutro (tipo_dia
--   'feriado', com o nome), sem falta e sem débito de jornada.
--   O feriado TRABALHADO segue pelo cálculo normal — é lá que a dobra e
--   a folga compensatória são decididas (PONTO-320/321).
--
-- (430) AJUSTE DE PONTO ACEITAVA MOTIVO VAZIO
--   A coluna motivo é NOT NULL, o que barra o nulo — mas string vazia (ou
--   um caractere) passava. Marcação alterada sem justificativa é indício
--   de manipulação do controle de jornada na fiscalização (Portaria MTP
--   671/2021), e o aprovador decidia no escuro.
--   Agora há CHECK de conteúdo mínimo. Ele entra como NOT VALID: passa a
--   valer para tudo que for gravado daqui em diante, sem reprovar
--   histórico antigo que ninguém pode mais corrigir.
--
-- (431) DOSSIÊ DE FISCALIZAÇÃO DUPLICAVA POR COMPETÊNCIA
--   Mandar montar de novo empilhava um segundo dossiê da mesma
--   competência, com conteúdo e hash diferentes. Na fiscalização é o pior
--   cenário: a empresa apresenta um e o auditor encontra o outro.
--   Agora a competência tem dossiê único: a remontagem ATUALIZA o
--   existente (peças, índice, hash e data), e um índice único garante a
--   regra no banco. Duplicatas anteriores são consolidadas na mais
--   recente — com cópia do que for removido guardada antes.
--
-- COMPATIBILIDADE: nenhuma escrita nova depende do formato do índice da
-- apuração diária (a chave com ou sem empresa) — o mesmo cuidado que
-- destravou a bancada em ambiente antigo.
-- =====================================================================
