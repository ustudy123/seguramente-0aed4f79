-- =====================================================================
-- Correção: 'empresa' não é valor do enum de escopo — e isso quebrava
-- a consulta de colaboradores comuns nas tabelas protegidas
--
-- A coluna perfil_permissoes.escopo é do tipo enum perfil_escopo_tipo
-- ('proprio_usuario', 'subordinados_diretos', ..., 'empresa_inteira').
-- A função perfil_permite_modulo — e a rotina PERFIL-004 — comparavam
-- com COALESCE(pp.escopo, 'empresa'), e 'empresa' NÃO é um valor do
-- enum. O Postgres rejeita o literal na hora de executar:
--   invalid input value for enum perfil_escopo_tipo: "empresa"
--
-- Consequências (reproduzidas em ambiente de ensaio com o enum real):
--   · a PERFIL-004 sempre terminava em 'erro' — o primeiro "erro" da
--     bateria em produção era ISTO, não tempo de execução;
--   · pior: como as políticas restritivas chamam perfil_permite_modulo,
--     um colaborador comum (sem papel de gestão) recebia ERRO ao
--     consultar as tabelas protegidas — inclusive os próprios
--     atestados. Administradores e gestores não eram afetados porque
--     a função retorna antes de chegar na comparação quebrada.
--
-- A correção: comparar como TEXTO (pp.escopo::text), que funciona
-- igual para enum e para texto. A semântica não muda: escopo diferente
-- de 'proprio_usuario' continua valendo como acesso amplo.
-- =====================================================================
