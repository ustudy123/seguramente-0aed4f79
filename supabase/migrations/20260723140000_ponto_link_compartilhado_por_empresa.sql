-- =====================================================================
-- LINK DE PONTO COMPARTILHADO: ESCOPO POR EMPRESA (não por tenant)
-- =====================================================================
-- Ajuste sobre a 20260723130000. O modelo é 1 tenant -> N empresas
-- (empresa_cadastro.tenant_id; o seletor do topo troca empresaAtivaId, não
-- o tenantId). Um link compartilhado por TENANT misturava colaboradores de
-- empresas distintas (ex.: um contador com 10 empresas-clientes teria 1 link
-- para todas, e o CPF resolveria em qualquer uma). Agora o link compartilhado
-- é POR EMPRESA: cada empresa tem o seu, e o CPF só resolve dentro da empresa
-- daquele link.
-- =====================================================================

-- 1) Coluna empresa_id em ponto_links (nullable; links por-colaborador legados
--    seguem com null). Hoje a tabela não tem empresa_id.
ALTER TABLE public.ponto_links
  ADD COLUMN IF NOT EXISTS empresa_id uuid REFERENCES public.empresa_cadastro(id);

-- 2) Remove link(s) compartilhado(s) tenant-wide criados sem empresa (teste).
--    Serão regerados por empresa pela UI.
DELETE FROM public.ponto_links
WHERE tipo = 'compartilhado' AND empresa_id IS NULL;

-- 3) Índice único passa a ser por (tenant_id, empresa_id): 1 link
--    compartilhado por empresa.
DROP INDEX IF EXISTS public.uq_ponto_links_compartilhado_por_tenant;
CREATE UNIQUE INDEX IF NOT EXISTS uq_ponto_links_compartilhado_por_empresa
  ON public.ponto_links (tenant_id, empresa_id)
  WHERE tipo = 'compartilhado';

-- 4) Resolver de CPF agora recebe a EMPRESA do link e filtra por ela -----
DROP FUNCTION IF EXISTS public._ponto_resolver_colaborador_cpf(uuid, text);
...
REVOKE EXECUTE ON FUNCTION public._ponto_resolver_colaborador_cpf(uuid, uuid, text) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public._ponto_resolver_colaborador_cpf(uuid, uuid, text) TO authenticated;

-- 5) RPCs _cpf passam a resolver pela empresa do link (v_link.empresa_id) ----
...
BEGIN
  ...
  SELECT * INTO v_colab FROM public._ponto_resolver_colaborador_cpf(v_link.tenant_id, v_link.empresa_id, p_cpf);
  IF v_colab.colaborador_id IS NULL THEN
    RETURN json_build_object('error', 'CPF não encontrado ou colaborador sem ponto ativo nesta empresa. Confira os números e tente novamente.');
  END IF;
...
BEGIN
  ...
  SELECT * INTO v_colab FROM public._ponto_resolver_colaborador_cpf(v_link.tenant_id, v_link.empresa_id, p_cpf);
  IF v_colab.colaborador_id IS NULL THEN
    RETURN json_build_object('error', 'CPF não encontrado ou colaborador sem ponto ativo nesta empresa.');
  END IF;
...
BEGIN
  ...
  BEGIN
    INSERT INTO public.ponto_marcacoes (
      ...
      v_link.tenant_id, v_link.empresa_id, v_colab.colaborador_id, v_colab.colaborador_nome, v_colab.colaborador_cpf,
...
-- 6) Ajustes pelo CPF: também resolvem pela empresa do link ---------------
