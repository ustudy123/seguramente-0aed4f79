-- =========================================================
-- 13º Salário — fundação do cálculo: avos e médias apurados do sistema
--
-- PROBLEMA: o 13º era digitado. A tela (Financeiro › 13º Salário) pedia
-- "meses trabalhados" e "média de variáveis" como dois números soltos, e
-- calcular13 (src/lib/folha/calculos.ts) apenas aritmetizava o que fosse
-- digitado. Ninguém lia data de admissão, faltas, afastamento nem a folha
-- do ano. O banco aceitava até 15 meses num ano de 12. Sem memória, nada
-- se reproduz e nada se audita.
--
-- ENTREGA (Entrega 1 do plano do módulo — o núcleo do cálculo):
--   1) decimo_terceiro_config — parâmetros por empresa, com vigência;
--   2) decimo_terceiro_avos(...) — apura os avos da Lei 4.090/1962
--      (1/12 por mês, fração >= 15 dias) a partir da admissão, das faltas
--      injustificadas do ponto e dos afastamentos, com a memória mês a mês;
--   3) decimo_terceiro_media_variaveis(...) — média das variáveis do ano
--      pelas rubricas marcadas com incide_13, com a memória competência a
--      competência;
--   4) decimo_terceiro_apurar(...) — junta as duas e devolve a base;
--   5) folha_13_calculo passa a registrar a ORIGEM (apurado x digitado) e
--      a recusar avos impossíveis.
--
-- DECISOES DO DONO DO PRODUTO (03/09/2026) — todas parametrizáveis:
--   * media_divisor = 'avos_apurados': a média divide pelos meses que
--     geraram avo (quem foi admitido em maio não é dividido por 12);
--   * afastamento_regra = 'previdenciario_suspende': afastamento com
--     benefício do INSS (B31/B91/B92/B32) deixa de contar dia trabalhado
--     a partir do 16º dia — os primeiros 15 são do empregador. Maternidade
--     e demais afastamentos INTEGRAM os avos;
--   * adiantamento_base = 'proporcional_apurado': a 1ª parcela é 50% do
--     13º proporcional apurado (a alternativa legal clássica — 50% da
--     remuneração do mês anterior — fica disponível no parâmetro).
--   A vigência fica gravada na memória para o cálculo não mudar o passado.
--
-- Requisitos YE-DP-13-001: RF-001, RF-002, RN-001, RN-002, RN-008,
-- CA-001, CA-002, RNF-001/002/007.
-- Casos de teste: DEC13-001, DEC13-002, DEC13-003, DEC13-020, DEC13-021.
--
-- Somente leitura sobre ponto, afastamentos, admissões e folha: as funções
-- de apuração não gravam nada.
-- =========================================================

SET lock_timeout = '10s';

-- ── 1. Parâmetros do 13º (por empresa, com vigência) ──────────────────────
...
BEGIN
    IF p_tenant IS NULL OR v_cpf = '' OR p_ano IS NULL THEN
        RETURN jsonb_build_object(
            'avos', 0, 'meses', '[]'::jsonb,
            'avisos', to_jsonb(ARRAY['Informe colaborador e ano-base para apurar os avos.'])
        );
    END IF;
...
BEGIN
    IF p_tenant IS NULL OR v_cpf = '' OR p_ano IS NULL THEN
        RETURN jsonb_build_object(
            ...
            'avisos', to_jsonb(ARRAY['Informe colaborador e ano-base para apurar a média.'])
        );
    END IF;
