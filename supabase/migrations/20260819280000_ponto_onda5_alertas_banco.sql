-- ============================================================================
-- ONDA 5 (parte 3) — Alertas de vencimento e de teto de acúmulo do banco
-- PONTO-355 / PONTO-356
--
-- Dois avisos que faltavam no banco de horas:
--   (355) VENCIMENTO PRÓXIMO: hoje o RH só descobre o saldo vencido quando ele
--         já virou passivo de hora extra (CLT art. 59, §5º). Falta o aviso
--         antecipado. Rotina que, X dias antes do prazo_compensacao (parte 2),
--         alerta com a ação sugerida — programar compensação ou pagar.
--   (356) TETO DE ACÚMULO: o campo limite_acumulo_horas da configuração é
--         decorativo — o gestor define um teto, o colaborador passa dele e nada
--         acontece. Passa a ser comparado com o saldo e a gerar alerta do
--         excedente.
--
-- Uma função de monitor (somente insere alertas, idempotente), no mesmo padrão
-- dos monitores das ondas 2 e 4. Nada roda sozinho, sem gatilho em tabela
-- quente, sem tocar no motor de saldo. Reaproveita ponto_banco_regime_vigente
-- (parte 1) para o teto do regime.
-- ============================================================================
