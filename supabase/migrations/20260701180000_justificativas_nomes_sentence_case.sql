...
-- [GUARDA DE AMBIENTE] Este arquivo tem carimbo retroativo (01/07) e ordena
-- ANTES da migration que cria a coluna "sistema" (24/07). Em banco novo, só
-- roda quando a coluna existir — e aí as justificativas já nascem certas.
DO $prodseed$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns
                 WHERE table_schema='public' AND table_name='ponto_justificativas'
                   AND column_name='sistema') THEN
    RAISE NOTICE 'ponto_justificativas.sistema ainda não existe: renomeações puladas.';
    RETURN;
  END IF;
