-- =====================================================================
-- PONTO — o espelho para de prometer 50% onde o credito e de 1 por 1
--
-- DE ONDE VEIO
-- Auditoria de fechamento de 01/09/2026 sobre uma competencia real. No dia
-- a dia do espelho, cada dia com sobra saia rotulado assim:
--
--     Soma Banco Horas 50%
--
-- e creditava, no banco, exatamente os minutos trabalhados a mais. Um por
-- um. O "50%" nao entrava em conta nenhuma — era so a palavra.
--
...
--   * hora COMPENSADA no banco: e trocada na exata medida, uma por uma
--     (§2º) — o acrescimo nao existe aqui, porque nao ha pagamento.
--
-- Escrever "50%" ao lado de um credito de 1 por 1 e prometer no documento
-- o que a conta nao faz. O trabalhador assina um espelho que diz 50%,
-- confere o saldo e encontra 1 por 1. E o documento que ele assina.
--
-- A CORRECAO
-- O rotulo passa a dizer o que a conta faz, e a distinguir os dois casos:
--
--   * COM regime de compensacao vigente, a sobra vai para o banco e o dia
--     sai como "Soma Banco Horas (1:1)" — sem percentual, porque nao ha;
--   * SEM regime vigente nao existe banco: as horas sao devidas em
--     dinheiro, e ai sim o dia sai como "Hora extra 50%" (ou 100% em
--     domingo e feriado, Sumula 146 do TST) — o percentual aparece
--     exatamente onde ele vale.
--
-- Para o documento saber em qual dos dois casos esta, a fonte unica do
-- banco de horas passa a informar se ha regime vigente (tem_regime). Ela ja
-- consultava isso para zerar o banco quando nao ha instrumento; agora
-- devolve a informacao em vez de guarda-la.
--
-- NENHUM NUMERO MUDA. O credito ja era de 1 por 1 e continua sendo; o que
-- muda e o documento parar de dizer outra coisa. A troca do rotulo em si
-- acontece na tela, e chega com o Publicar no Lovable.
-- =====================================================================

SET lock_timeout = '10s';

-- ---------------------------------------------------------------------
-- 1) A fonte unica passa a dizer se ha regime de compensacao vigente
--    (a coluna e nova, entao a funcao precisa ser recriada; ela e somente
--    leitura e nasceu nesta mesma leva de correcoes)
-- ---------------------------------------------------------------------
...
DO $fim$
BEGIN
  RAISE NOTICE 'A hora que vai para o banco vale 1 por 1, e a fonte diz quando ha regime.';
END $fim$;
