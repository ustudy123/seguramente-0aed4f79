-- =====================================================================
-- Aprovar ajuste de ponto volta a funcionar
--
-- A rotina processar_ajuste_ponto foi reescrita em 07/08 para incluir a
-- segregação de funções da Portaria 671 (ninguém aprova o próprio ponto,
-- auto-lançamento exige justificativa). A regra nova está correta e é
-- mantida aqui — mas o mesmo arquivo trocou o trecho que grava marcações
-- por uma versão que nunca poderia ter funcionado:
--
--   · gravava em data_hora / tipo / origem / observacao / ajuste_id /
--     criado_por — SEIS colunas que não existem em ponto_marcacoes (a
--     tabela usa data_marcacao + hora_marcacao + tipo_marcacao +
--     origem_marcacao + created_by). Erro visto pelo usuário:
--     'column "data_hora" of relation "ponto_marcacoes" does not exist';
--   · chamava _ponto_grava_abono com 9 argumentos, para uma função que
--     recebe 6 — então ajustes de justificativa/abono quebravam também;
--   · perdeu duas travas conquistadas a duro custo: a correção só apaga
--     quando há hora_original explícita (sem isso, uma inclusão varre as
--     batidas do dia) e a verificação que evita batida duplicada no
--     mesmo minuto.
--
-- Esta migration mantém 100% da regra de segregação de funções e devolve
-- o bloco de marcações à versão íntegra de 24/07, com os nomes de coluna
-- reais.
--
-- E corrige um QUARTO defeito, que só aparece depois dos três primeiros:
-- gravada a batida, o gatilho de inserção chama a reordenação automática
-- de rótulos (ponto_reordena_tipos_dia), que faz UPDATE de tipo_marcacao
-- — e cai na trava de imutabilidade criada no mesmo dia 07/08
-- (ponto_bloquear_update_marcacao). A aprovação inteira era recusada com
-- "Marcação original é imutável (Portaria MTP 671/2021)". A rotina de QA
-- PONTO-351 já registrava esse achado. Correção abaixo: um contexto
-- próprio (app.ponto_reordena) que libera EXCLUSIVAMENTE a troca do
-- rótulo entrada/saída — horário, data, CPF, colaborador e o selo de
-- marcação original continuam imutáveis como a Portaria exige.
-- =====================================================================
