...
BEGIN
  FOR rec IN
    SELECT DISTINCT tenant_id, colaborador_cpf, competencia
  ...
  LOOP
    ...
    EXCEPTION WHEN OTHERS THEN
      RAISE NOTICE 'Falha ao reapurar % % %: %', rec.tenant_id, rec.colaborador_cpf, rec.competencia, SQLERRM;
    END;
  END LOOP;
END $reapura$;
...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'ponto_saldo_dias_competencia') THEN
    RAISE EXCEPTION 'FALHOU: função de saldo diário não existe.';
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM pg_proc
    WHERE proname = 'ponto_saldo_dias_competencia'
      AND prosrc ILIKE '%abs(v_diff)%'
  ) THEN
    RAISE EXCEPTION 'FALHOU: tolerância de 10 minutos no saldo final não foi aplicada.';
  END IF;

  RAISE NOTICE 'OK: tolerância de 10 minutos no saldo final aplicada e bancos reapurados.';
END $verifica$;