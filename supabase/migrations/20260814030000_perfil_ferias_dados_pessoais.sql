-- =====================================================================
-- Férias entram na camada de perfil — e o colaborador passa a ver só o
-- que é dele
--
-- Achado na conferência do módulo Férias contra a análise de requisitos
-- (YE-DP-FERIAS-001), registrada em docs/conferencia_ferias_requisitos.md:
--
-- As tabelas de férias liberam leitura por "tenant + vínculo com a
-- empresa" e nada mais. Qualquer usuário autenticado da empresa alcança,
-- pela API, os dados de férias de TODOS os colegas — e ferias_solicitacoes
-- guarda salario_base, valor_ferias, valor_terco e valor_total_bruto.
-- Na prática: salário de colega exposto a quem tiver login.
--
-- O documento exige o contrário em dois lugares: a matriz da seção 6
-- restringe o colaborador a "só próprios dados", e a seção 22 (LGPD)
-- pede minimização e acesso do colaborador aos próprios dados.
--
-- Férias tinha ficado inteiramente de fora da camada de perfil criada
-- pelo parecer de perfis de acesso (07/08) — 13 tabelas sensíveis,
-- nenhuma delas de férias. E a rotina que vigia isso (PERFIL-003) não
-- enxergava o módulo: ela varre atestado%, %_saude%, %psicossocial% e
-- documentos. Mesmo ponto cego do questionário psicossocial, em outro
-- canto da base.
--
-- COMO FICA
--   • Quem administra férias (superadmin, gestor para cima, tipo
--     administrador/gestor, ou perfil com o módulo 'ferias' ou
--     'colaboradores' em escopo amplo) continua vendo tudo, igual a hoje.
--   • Todo o resto passa a ver apenas as próprias linhas, pelo CPF.
--
-- O QUE NÃO MUDA
--   • Escrita: as políticas abaixo são de SELECT. A exposição é de leitura.
--   • Assinatura do aviso por link público: a página lê por função de
--     borda (service_role) e políticas RESTRICTIVE só valem para os papéis
--     que nomeiam — aqui, `authenticated`. O caminho do link segue intacto.
--   • Telas de RH/DP: nada a mudar no front. Quem passa no
--     perfil_permite_modulo enxerga o mesmo conjunto de sempre.
--
-- RESSALVA REGISTRADA (não corrigida aqui, de propósito)
--   O catálogo de perfis oferece o módulo 'ferias', mas o mapa de rotas
--   (src/lib/moduleAccess.ts) associa /ferias a 'colaboradores'. Quem
--   receber só "Férias" no perfil não alcança a tela. Por isso a política
--   aceita OS DOIS módulos: assim ninguém perde acesso enquanto a
--   inconsistência do mapa não for decidida com os perfis reais em mãos.
-- =====================================================================
...
BEGIN
  ...
  IF v_n < 7 THEN
    RAISE EXCEPTION 'Camada de perfil incompleta em Férias: % de 7 políticas.', v_n;
  END IF;
  RAISE NOTICE 'OK: 7 tabelas de férias com leitura condicionada ao perfil.';
END $verifica$;
