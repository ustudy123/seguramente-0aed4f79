-- ============================================================================
-- ONDA 10 (parte 3) — Radar de cobertura de turno (ESC-021)
--
-- Operação por turnos vive de cobertura: um turno previsto sem colaborador
-- disponível (porque quem estava atribuído entrou de férias, se afastou ou foi
-- desligado no meio do ciclo) precisa aparecer ANTES do dia — com alerta ao
-- gestor e ação de cobertura — e não ser descoberto com o posto vazio. Hoje o
-- sistema TEM os ingredientes (atribuições de escala com vigência, afastamentos,
-- férias, desligamentos) e não os cruza: a solução de última hora vira dobra de
-- turno, que estoura interjornada e hora extra (o que PONTO-080/092 vigiam).
--
...
-- atribuídos estão indisponíveis — turno descoberto. Um listador read-only
-- (para a tela) e um monitor que gera o alerta ao gestor, idempotente por
-- escala/dia. Não bloqueia nada, não altera o motor de saldo, a apuração, o
-- espelho nem o fechamento. Sem gatilho em tabela quente (é monitor agendável).
-- ============================================================================

-- Listador read-only: os turnos descobertos previstos na janela (para a tela).
...
AS $function$
  ...
  -- (escala, dia, colaborador atribuido vigente) e se esta indisponivel no dia
  cobertura AS (
  ...
  HAVING bool_and(c.indisponivel) = true   -- turno descoberto: ninguem disponivel
  ORDER BY c.d, c.nome;
$function$;
...
-- Monitor: gera o alerta ao gestor para cada turno descoberto previsto.
