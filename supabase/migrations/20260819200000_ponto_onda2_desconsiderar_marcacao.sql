-- ============================================================================
-- ONDA 2 (parte 5) — Correção por acréscimo: desconsiderar, nunca apagar
-- PONTO-004 (resto)
--
-- A imutabilidade da marcação já bloqueia UPDATE/DELETE direto (PONTO-004). O
-- que restava: os RPCs excluir_marcacao_ponto e processar_ajuste_ponto (tipo
-- "correcao") ainda APAGAVAM a marcação original pelo flag de sessão
-- app.allow_ponto_delete — correção por substituição, que destrói a prova.
--
-- O QUE MUDA
--   A batida original nunca é apagada. Passa a ser DESCONSIDERADA: fica no
--   acervo (e na cadeia de hash), marcada como desconsiderada, com motivo e
--   responsável, e SAI do cálculo do dia. A necessidade operacional (batida
--   duplicada/incorreta) continua atendida; a prova deixa de ser destruída.
--   (Portaria 671/2021 veda apagar; CLT art. 74; Súmula 338 do TST.)
--
--   · colunas de desconsideração em ponto_marcacoes;
--   · desconsiderar_marcacao_ponto(id, motivo) — o RPC próprio;
--   · excluir_marcacao_ponto passa a DELEGAR (compat: o antigo "Excluir" agora
--     desconsidera, sem apagar);
--   · processar_ajuste_ponto (correcao) desconsidera em vez de apagar;
--   · a consolidação, a reordenação e o corte da virada IGNORAM as
--     desconsideradas (a cadeia de hash e o NSR as mantêm — a prova fica).
--
-- A parte de TELA (o botão "Excluir" vira "Desconsiderar") vai no frontend e
-- exige Publicar no Lovable — descrita à parte na entrega.
-- ============================================================================
