-- ============================================================================
-- ONDA 2 (parte 1) — Cadeia de hash encadeado + verificação
-- PONTO-191
--
-- Hoje cada marcação tem hash próprio (sha256 do conteúdo), mas os hashes NÃO
-- se encadeiam: remover uma linha inteira não quebra nada, e nenhuma rotina
-- confere. Encadeamento verificado é o que transforma "não editamos" em prova
-- (registro tipo 7 do AFD; Portaria MTP 671/2021, Anexo IX).
--
-- O QUE MUDA
--   (1) Coluna hash_anterior em ponto_marcacoes.
--   (2) O hash de cada marcação nova passa a INCORPORAR o hash da marcação
--       anterior da mesma sequência (por NSR, dentro do tenant/estabelecimento).
--       O append é de COALESCE(hash_anterior, '') — vazio para a primeira da
--       cadeia e para as marcações legadas, o que mantém o hash delas IDÊNTICO
--       ao de antes (append de '' é no-op no sha256). Assim a mudança é
--       retrocompatível: nada precisa ser reprocessado.
--   (3) Rotina ponto_verificar_cadeia_hash: recomputa o hash de cada marcação
--       (detecta adulteração de conteúdo) e confere o encadeamento e a
--       continuidade da NSR (detecta remoção de linha), devolvendo as quebras.
--       Uma companheira agendável (ponto_cadeia_hash_monitorar) alerta o RH.
--
-- Defensivo: a geração do hash nunca bloqueia a inserção da marcação — se a
-- leitura do hash anterior falhar, segue sem encadear (hash_anterior nulo).
-- ============================================================================
