-- =========================================================
-- QA — Ouvidoria: rotinas executaveis pelo motor (Executar testes)
--
-- Dos 30 casos OUV documentados, este arquivo torna EXECUTAVEIS pelo motor
-- (qa_rodar_bateria) os que dependem de uma regra do BANCO — o motor roda
-- SQL somente-leitura, entao ele nao clica na tela, mas VERIFICA a regra que
-- sustenta cada fluxo (coluna obrigatoria, CHECK de vocabulario, politica de
-- RLS). Se alguem enfraquecer essas protecoes, o caso vira vermelho.
--
-- Os 9 casos abaixo passam a nivel 'api' e ganham rotina; os demais (form,
-- botoes, IA, contadores, cards) seguem 'e2e' — so a tela testa.
--
-- Cada rotina devolve public.qa_retorno (passou | falhou | nao_implementado |
-- erro) e nao escreve nada. Se a tabela nao existir no ambiente, devolve
-- nao_implementado em vez de quebrar.
--
-- Idempotente: CREATE OR REPLACE + ON CONFLICT DO UPDATE.
-- =========================================================
