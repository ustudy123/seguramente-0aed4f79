-- ============================================================================
-- ONDA 6 (parte 1) — Geração transacional dos espelhos da competência
-- PONTO-194
--
-- Hoje os espelhos nascem LINHA A LINHA por um caminho de tela/edge que o banco
-- não conhece: uma falha no meio deixa a competência com espelho de metade dos
-- colaboradores e metade sem — pior que ausente, porque parece completo. Não há
-- função transacional de geração (a rotina PONTO-194 vinha passando por acaso,
-- casando com ponto_reabrir_competencia, que só arquiva espelhos ao reabrir).
--
-- O QUE FAZ (aditivo): ponto_gerar_espelhos_competencia gera os espelhos da
-- competência numa ÚNICA função (tudo-ou-nada — como toda função plpgsql, roda
-- em uma transação: falha no meio desfaz tudo). Para cada colaborador com ponto
-- na competência, compõe os totais a partir da apuração canônica
-- (ponto_espelho_resumo) e faz UPSERT em ponto_espelhos. Regenerar preserva a
-- ciência já dada (status, confirmação, assinatura, ressalva) — só atualiza os
-- números; a invalidação de ciência é o fluxo de reabertura (onda 2).
--
-- GARANTIAS
--   · Não altera o motor de saldo nem a apuração; só compõe e grava o espelho.
--   · Não é chamada por gatilho — o fechamento (parte 2) a invoca. Idempotente.
-- ============================================================================
