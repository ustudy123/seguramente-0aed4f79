-- Atualizar a função de consolidação para ser mais inteligente com duplicatas e registros próximos
-- Desabilitamos temporariamente o trigger de bloqueio de delete se precisássemos limpar, 
-- mas como a função de consolidação já filtrará, vamos apenas atualizar a lógica.
...
BEGIN
  ...
  -- Usamos uma lógica de agregação para filtrar marcações muito próximas ou duplicadas
  FOR r IN
    WITH marcacoes_ordenadas AS (
      SELECT 
        ...
        LAG(tipo_marcacao) OVER (ORDER BY hora_marcacao) as last_tipo_geral
    ...
    marcacoes_unicas AS (
      ...
      WHERE (last_hora_mesmo_tipo IS NULL OR (hora_marcacao - last_hora_mesmo_tipo > interval '1 minute'))
    )
  ...
  -- Se sobrar uma entrada sem saída, ou o número de marcações filtradas for ímpar, é ajuste pendente
