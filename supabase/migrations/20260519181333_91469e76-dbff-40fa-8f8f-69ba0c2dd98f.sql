...
DECLARE
  v_cpf text;
  v_id_text text := _admissao_id::text;
  v_total int := 0;
...
BEGIN
  ...
  IF v_cpf IS NOT NULL AND length(regexp_replace(v_cpf, '\D', '', 'g')) > 0 THEN
    SELECT count(*) INTO v_count FROM public.colaboradores WHERE regexp_replace(cpf, '\D', '', 'g') = regexp_replace(v_cpf, '\D', '', 'g');
    IF v_count > 0 THEN v_total := v_total + v_count; v_detalhes := v_detalhes || jsonb_build_object('cadastro_colaborador', v_count); END IF;
  END IF;
