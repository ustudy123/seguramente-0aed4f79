-- =====================================================================
-- PONTO — 421 (folga compensatória) e 410 (batida vence a declaração)
--
-- (421) FOLGA COMPENSATÓRIA NÃO EXISTIA COMO REGISTRO
--   Compensar é o propósito do banco de horas (CLT art. 59, §2º), e não
--   havia como registrar a folga: o dia sem marcação virava FALTA, e o
--   débito entrava no extrato junto com "atrasos, faltas e saídas
--   antecipadas". O efeito era duplamente caro — a empresa dava a folga
--   E mantinha o saldo positivo (que seria pago de novo no vencimento),
--   enquanto o colaborador que estava justamente compensando aparecia
--   com falta, arriscando o desconto e o DSR da semana.
--
--   Agora existe ponto_registrar_folga_compensatoria: grava o dia como
--   folga (neutro, sem falta) e lança no banco a movimentação do tipo
--   'compensacao' — o tipo que a apuração já soma em compensados_minutos,
--   separada dos débitos por ausência. Idempotente: registrar a mesma
--   folga duas vezes não debita duas vezes. A consolidação passa a
--   respeitar o dia de folga em vez de reescrevê-lo como falta.
--
-- (410) NÃO ERA DEFEITO — A SONDA OLHAVA O LUGAR ERRADO
--   A regra da Súmula 338, III (a marcação real prevalece sobre a
--   pré-assinalação) JÁ ESTÁ implementada, e corretamente: o gatilho
--   ponto_diario_pre_assinalacao marca intervalo_origem = 'marcado'
--   sempre que há almoço batido, e só aplica o declarado no silêncio.
--   A sonda procurava a lógica dentro de _ponto_calc_dia — um detalhe de
--   implementação, não o comportamento — e por isso reprovava um sistema
--   que atende à regra. Aqui ela passa a MEDIR: cria um dia com almoço
--   batido e outro sem, e confere a origem gravada em cada um.
--
-- COMPATIBILIDADE: nenhuma escrita nova depende do formato do índice da
-- apuração diária (chave com ou sem empresa).
-- =====================================================================
