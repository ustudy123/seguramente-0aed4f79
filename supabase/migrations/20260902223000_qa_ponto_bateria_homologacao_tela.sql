-- =========================================================
-- QA PONTO — Bateria de Homologação do Ponto (roteiro do testador, 31 casos)
--
-- Módulo jornada-rotina/ponto. Este roteiro é o LADO DE TELA (nivel e2e) da
-- homologação do Ponto: 31 casos em 9 famílias (A..I) que dizem, para uma
-- pessoa, onde ir, o que fazer e o que precisa acontecer. Complementa as
-- sondas de motor já documentadas (PONTO-4xx, nivel api): mesma matéria legal,
-- outro nível de teste — aqui se confere na tela.
--
-- Códigos PONTO-HOM-<familia><n> preservam os rótulos A1..I3 que o testador
-- usa no roteiro (o rodapé pede "me diga o código, por exemplo D6").
--
-- Níveis:
--   e2e (tela) .... 28 casos — conferência na tela/relatório.
--   api (banco) ...  3 casos — C1, C2 e F2, conferidos no SQL Editor de teste.
--
-- Cruzamentos com as sondas de motor (nas observações do caso):
--   C1 -> PONTO-450 (motor de vigilâncias roda inteiro e agendado)
--   C2 -> PONTO-451 (rodar duas vezes não duplica alerta)
--   E3 -> PONTO-431 (remontar dossiê não duplica)
--   F3 -> PONTO-460 (tentativas em série no link são contidas)
--
-- Regra da casa: caso e2e sem spec Cypress só gera AVISO na guarda de
-- cobertura (não reprova). Caso api sem rotina fica nao_implementado (idem).
