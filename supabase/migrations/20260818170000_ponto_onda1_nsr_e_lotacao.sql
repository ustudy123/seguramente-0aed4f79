-- =====================================================================
-- PONTO — ONDA 1 (parte 1): NSR e histórico de lotação
-- =====================================================================
-- Duas mudanças de FORMA. São baratas agora e caras depois: tudo que as
-- ondas seguintes constroem (AFD, AEJ, dossiê, apuração por vínculo) se
-- apoia nelas.
--
--   PONTO-210  NSR — numeração sequencial da marcação, imutável
--   PONTO-395  histórico de lotação com vigência
--
-- Nenhuma altera cálculo de jornada. São estruturas novas ao lado do que
-- já existe; nenhuma função de apuração passa a lê-las nesta migration.
--
-- O PREENCHIMENTO HISTÓRICO DO NSR NÃO ESTÁ AQUI, de propósito. Cada
-- UPDATE em ponto_marcacoes dispara o gatilho de auditoria, que grava uma
-- linha com o JSON inteiro do antes e do depois. Preencher a tabela toda
-- dentro da migration inflaria a auditoria de produção sem ninguém olhando
-- e correria risco de estourar tempo. O preenchimento vive em
-- docs/script_ponto210_backfill_nsr.sql, em lotes e re-executável.
-- =====================================================================
