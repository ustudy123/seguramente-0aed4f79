-- ============================================================================
-- ONDA 7 (parte 2) — AEJ: Arquivo Eletrônico de Jornada (Portaria MTP 671/2021)
-- PONTO-211
--
-- O AEJ é a saída OBRIGATÓRIA do "programa de tratamento" na Portaria 671 (ele
-- substituiu o AFDT/ACJEF da 1510/2009) e a peça que a fiscalização pede junto
-- com o AFD. Hoje não existe em lugar nenhum do banco — nem função, nem coluna,
-- nem tabela. Sem ele, a empresa não tem como entregar a jornada TRATADA
-- (contrato + marcações tratadas + apuração) num arquivo assinado.
--
-- O QUE FAZ (aditivo)
--   (1) ponto_arquivos_aej: o arquivo gerado, arquivado — empregador, período,
--       conteúdo (registros tipados), versão estruturada, assinatura (hash) e
--       contagens. Com a trava do cercado (PONTO-270) e RLS por tenant.
--   (2) ponto_gerar_aej(tenant, empresa, competencia): gera o AEJ da competência
--       A PARTIR DA APURAÇÃO (ponto_espelhos) + o contrato (admissoes) + as
--       marcações TRATADAS (ponto_marcacoes não desconsideradas, com origem e
--       NSR), no leiaute de registros tipados da Portaria 671, assinado por hash.
--       Idempotente (refaz o arquivo da competência/empresa).
--   (3) ponto_aej_extrair(tenant, empresa, competencia): devolve o arquivo
--       arquivado (conteúdo + assinatura) para download/fiscalização.
--
-- Leiaute dos registros (um por linha, campos separados por '|'), na estrutura
-- do AEJ da Portaria 671 — o tratamento entrega contrato, marcações e apuração:
--   1 | tipo_id_empregador | cnpj_cpf | cei_caepf | razao_social | ini | fim   (cabeçalho)
--   2 | cpf | nome | data_admissao | jornada_contratual                       (contrato do trabalhador)
--   3 | cpf | data | hora | tipo_marcacao | origem | nsr                       (marcação tratada)
--   4 | cpf | competencia | normais | he50 | he100 | adic_not | faltas | atrasos | dsr  (apuração)
--   9 | qtd_reg_2 | qtd_reg_3 | qtd_reg_4 | total_registros                    (trailer)
-- A assinatura (hash sha256 do conteúdo) é gravada à parte; o formato posicional
-- oficial é aplicado na exportação, sobre este mesmo conteúdo tratado.
--
