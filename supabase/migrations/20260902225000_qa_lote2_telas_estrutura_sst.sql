-- =========================================================
-- QA — Lote 2 de telas: estrutura organizacional + Compliance SST
--
-- Mais quatro módulos só-motor ganham a bateria estrutural de tela (e2e):
-- o módulo monta, a lista/abas aparecem, o formulário principal abre, os
-- vazios orientam e a exclusão pede confirmação. Fiéis à tela real.
--
--   Departamentos ...... estrutura-organizacional/departamentos    (DEPTO-TELA-*)  /cadastros/departamentos
--   Cargos ............. estrutura-organizacional/cargos           (CARGO-TELA-*)  /cadastros/cargos
--   Estabelecimentos ... estrutura-organizacional/estabelecimentos (ESTAB-TELA-*)  /cadastros/filiais
--   Compliance SST ..... saude-seguranca/compliance-sst            (CSST-TELA-*)   /compliance-sst
--
-- Prefixos -TELA- não colidem com as sondas de motor existentes.
...
BEGIN
  ...
  IF v_mod IS NULL THEN RAISE EXCEPTION 'Módulo estrutura-organizacional/departamentos não encontrado.'; END IF;
...
BEGIN
  ...
  IF v_mod IS NULL THEN RAISE EXCEPTION 'Módulo estrutura-organizacional/cargos não encontrado.'; END IF;
  SELECT count(*) INTO v_antes FROM public.qa_casos_teste WHERE modulo_id = v_mod;
...
BEGIN
  ...
  IF v_mod IS NULL THEN RAISE EXCEPTION 'Módulo estrutura-organizacional/estabelecimentos não encontrado.'; END IF;
  SELECT count(*) INTO v_antes FROM public.qa_casos_teste WHERE modulo_id = v_mod;
