-- ============================================================================
-- ONDA 11 (parte 3) — Ausência do art. 473 sem documento fica pendente (ESC-012)
--
-- O rol do art. 473 da CLT abona a falta, mas MEDIANTE comprovação: certidão de
-- óbito, de casamento, declaração de comparecimento, comprovante de doação de
-- sangue. Hoje o catálogo `ponto_justificativas` já marca os tipos que exigem
-- documento (`requer_anexo`) e a tela recebe essa flag — mas nada IMPEDIA o
-- abono sem documento: um gestor aprovava a justificativa e o abono se
-- consumava, com ou sem prova. Abonar sem documento é abrir mão de prova;
-- descontar às cegas seria descontar direito líquido (a certidão que chega
-- depois). O certo é ficar PENDENTE.
--
-- O QUE FAZ:
--   1) `processar_ajuste_ponto` passa a olhar `requer_anexo`: quando o abono
--      exige documento e o ajuste entra SEM anexo, o abono NÃO se consuma — o
--      ajuste fica `pendente_comprovacao`, com um PRAZO, e um alerta avisa o
--      colaborador e o DP. Não abona sem prova; não desconta às cegas.
--   2) Anexado o documento e reprocessado, o abono se efetiva normalmente
--      (o estado `pendente_comprovacao` é reprocessável).
--   3) `ponto_comprovacao_monitorar` varre as pendências de comprovação com
--      prazo vencido e alerta o DP ANTES de o dia virar desconto.
--
-- Nova coluna `ponto_ajustes.comprovacao_prazo` guarda a data-limite. Aditivo e
-- idempotente. Não altera o motor de saldo, a apuração, o espelho nem o
-- fechamento — só condiciona a consumação do abono à prova.
-- ============================================================================
