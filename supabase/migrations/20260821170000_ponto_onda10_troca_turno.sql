-- ============================================================================
-- ONDA 10 (parte 4) — Troca de turno com aprovação e recálculo (ESC-020)
-- Fecha a onda 10 (escalas).
--
-- Trocar o turno de dois colaboradores NÃO é editar duas linhas: a troca precisa
-- de APROVAÇÃO (gestor), REGISTRO (quem trocou com quem, quando) e RECÁLCULO dos
-- envolvidos — a interjornada de 11h (CLT art. 66) e o adicional noturno podem
-- mudar para os dois. Hoje não há estrutura: a troca vira edição manual de duas
-- linhas de ponto_escala_atribuicoes, sem aprovação, sem registro e sem simular a
-- interjornada — um passivo criado por boa vontade.
--
-- O QUE FAZ (aditivo): cria a tabela ponto_troca_turno (solicitação → aprovação →
-- efetivação, com trava do cercado) e o fluxo transacional:
--   - solicitar: registra a troca e SIMULA a interjornada dos dois ANTES de
--     consumar (marca risco quando a virada fica abaixo de 11h);
--   - aprovar / recusar: alçada do gestor registrada;
--   - efetivar: troca as atribuições em ponto_escala_atribuicoes de forma
--     transacional, PRESERVANDO o histórico de vigência (encerra as antigas e
--     cria as cruzadas; se a troca tem fim, restaura depois).
--
-- GARANTIAS: não altera o motor de saldo, a apuração, o espelho nem o fechamento.
-- A efetivação só mexe nas atribuições da escala (append, sem apagar história).
-- Aditivo e idempotente. SET lock_timeout na DDL.
-- ============================================================================
...
DECLARE
  a        RECORD;
  b        RECORD;
  ea       RECORD;   -- escala de origem de A
  eb       RECORD;   -- escala de origem de B
  v_gap_a  numeric;  -- interjornada de A ao assumir a escala de B
  v_gap_b  numeric;  -- interjornada de B ao assumir a escala de A
...
BEGIN
  ...
  -- Interjornada (art. 66): gap entre a saida da escala de ORIGEM (vespera) e a
  -- entrada da escala de DESTINO (dia da troca). Abaixo de 11h = risco.
...
-- ── Aprovar / recusar (alçada do gestor) ──
...
-- ── Efetivar: troca as atribuições preservando o histórico de vigência ──
...
BEGIN
  ...
  -- Encerra as atribuicoes atuais preservando a historia (nao apaga).
  ...
  -- Cria as atribuicoes cruzadas para o periodo da troca.
  ...
  -- Troca com prazo: restaura as atribuicoes originais apos o periodo.
