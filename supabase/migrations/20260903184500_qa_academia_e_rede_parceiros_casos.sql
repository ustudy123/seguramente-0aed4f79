-- =========================================================
-- QA — Academia e Rede de Parceiros: primeira documentacao de casos
--
-- Os dois modulos estavam no catalogo com ZERO casos. Este arquivo abre as
-- familias ACAD e PARC com casos de TELA (nivel e2e), derivados das telas
-- reais:
--   · Academia (pagina Academia, so Super Admin): CRUD de Categoria,
--     Treinamento, Modulo e Aula no admin; e o lado do aluno (explorar,
--     filtrar, favoritar, assistir com progresso/XP, concluir com badge).
--   · Rede de Parceiros (Marketplace): cadastro e validacao de profissional
--     (documentos + selfie, analise antes de ficar visivel), servicos e
--     pacotes, contratacao -> execucao -> avaliacao, denuncia e afiliados.
--
-- Cobertura pedida: CRUD e caminho feliz pelo menos, com alguns alternativos
-- e excecoes. Cada caso tem pre-condicoes, passos e resultado esperado.
--
-- Regra da casa: caso e2e documentado sem spec so gera aviso na guarda.
-- Idempotente: ON CONFLICT (codigo) DO NOTHING. So documenta.
-- =========================================================
...
BEGIN
  ...
  IF v_mod IS NULL THEN
    RAISE NOTICE 'Modulo academia ausente neste ambiente — casos ACAD nao inseridos.';
    RETURN;
  END IF;
  ...
  RAISE NOTICE 'Academia: casos antes=%, depois=% (esperado +15 na primeira execucao)', v_antes, v_depois;
END $acad$;
...
BEGIN
  ...
  IF v_mod IS NULL THEN
    RAISE NOTICE 'Modulo rede-parceiros ausente neste ambiente — casos PARC nao inseridos.';
    RETURN;
  END IF;
  SELECT count(*) INTO v_antes FROM public.qa_casos_teste WHERE modulo_id = v_mod;
  ...
  SELECT count(*) INTO v_depois FROM public.qa_casos_teste WHERE modulo_id = v_mod;
  RAISE NOTICE 'Rede de Parceiros: casos antes=%, depois=% (esperado +15 na primeira execucao)', v_antes, v_depois;
END $parc$;
