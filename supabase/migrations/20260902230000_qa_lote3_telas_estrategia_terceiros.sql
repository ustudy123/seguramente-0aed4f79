-- =========================================================
-- QA — Lote 3 (final) de telas: estratégia, terceiros e afastamentos
--
-- Fecha a rodada de telas dos módulos só-motor. Bateria estrutural de tela
-- (nivel e2e), fiel à tela real de cada um:
--
--   Planejamento Estratégico . planejamento-gestao/planejamento-estrategico (PLEST-TELA-*) /estrategia
--   Organograma .............. estrutura-organizacional/organograma          (ORG-TELA-*)   /estrategia?tab=organograma
--   Identidade / Cultura ..... planejamento-gestao/identidade-estrategica    (IDENT-TELA-*) /estrategia?tab=cultura
--   Prestadores .............. estrutura-organizacional/prestadores          (TERC-TELA-*)  /terceiros
--   Afastamentos ............. jornada-rotina/afastamentos                   (AFAST-TELA-*) /atestados (MOD-GAF)
--
-- Os três primeiros vivem na página /estrategia (trocada pelo parâmetro ?tab=).
-- Prefixos -TELA- não colidem com as sondas de motor.
-- Regra da casa: caso e2e sem spec Cypress só gera AVISO na guarda (não
-- reprova). Idempotente: ON CONFLICT (codigo) DO NOTHING. Só INSERE doc.
-- =========================================================
...
BEGIN
  ...
  IF v_mod IS NULL THEN RAISE EXCEPTION 'Módulo planejamento-gestao/planejamento-estrategico não encontrado.'; END IF;
...
BEGIN
  ...
  IF v_mod IS NULL THEN RAISE EXCEPTION 'Módulo estrutura-organizacional/organograma não encontrado.'; END IF;
  SELECT count(*) INTO v_antes FROM public.qa_casos_teste WHERE modulo_id = v_mod;
...
BEGIN
  ...
  IF v_mod IS NULL THEN RAISE EXCEPTION 'Módulo planejamento-gestao/identidade-estrategica não encontrado.'; END IF;
  SELECT count(*) INTO v_antes FROM public.qa_casos_teste WHERE modulo_id = v_mod;
...
BEGIN
  ...
  IF v_mod IS NULL THEN RAISE EXCEPTION 'Módulo estrutura-organizacional/prestadores não encontrado.'; END IF;
  SELECT count(*) INTO v_antes FROM public.qa_casos_teste WHERE modulo_id = v_mod;
