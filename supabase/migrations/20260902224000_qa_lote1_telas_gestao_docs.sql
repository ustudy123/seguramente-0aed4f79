-- =========================================================
-- QA — Lote 1 de telas: módulos só-motor ganham documentação de tela (e2e)
--
-- Quatro módulos com boa cobertura de MOTOR (sondas api) e ZERO documentação
-- de TELA passam a ter a bateria estrutural de tela (nivel e2e): o módulo
-- monta, as abas/seções abrem, os formulários principais aparecem e os vazios
-- orientam. É o mesmo passo que demos no Ponto — o lado que o testador confere
-- clicando, e a base para futuros testes de Cypress (doc antes do teste).
--
--   Metas ................ planejamento-gestao/metas         (METAS-TELA-*)
--   Plano de Ação ........ planejamento-gestao/plano-de-acao (PACAO-TELA-*)
--   Documentos ........... documentos-governanca/documentos  (DOCS-TELA-*)
--   Hub Contábil ......... documentos-governanca/hub-contabil(HUBC-TELA-*)
--
-- Prefixos -TELA- não colidem com as sondas de motor (MCHK/PLEV/DOC/HUB/...).
...
BEGIN
  ...
  IF v_mod IS NULL THEN RAISE EXCEPTION 'Módulo documentos-governanca/documentos não encontrado.'; END IF;
  SELECT count(*) INTO v_antes FROM public.qa_casos_teste WHERE modulo_id = v_mod;
