...
BEGIN
  ...
  -- Janela do vínculo com escala: usada para materializar dias SEM registro
  -- de ponto (faltas não lançadas), sem inventar dias fora do vínculo.
  ...
  FOR r IN
    SELECT g.d::date AS data,
           ...
           COALESCE(d.colaborador_id, v_colaborador_id) AS colaborador_id,
           d.entrada, d.saida
  ...
  LOOP
    ...
    -- Dia justificado em que o colaborador ainda assim bateu ponto e trabalhou:
    -- o dia nunca gera débito, mas o tempo trabalhado a mais deve virar crédito.
    ...
    -- Dia justificado com trabalho efetivo: só crédito, nunca débito.
