-- =========================================================
-- FIX URGENTE: aprovação de ajustes quebrada —
-- relation "public.usuario_tenants" does not exist
--
-- A validação multi-tenant da processar_ajuste_ponto referia
-- uma tabela inexistente (suposição baseada numa função morta
-- do histórico de migrations). Os mecanismos reais de vínculo:
--   1) profiles.tenant_id (a mesma base que o RLS usa para a
--      gestora ENXERGAR os ajustes — caso da Cacilda)
--   2) usuario_vinculos (vínculo usuário→empresa do tenant)
--
-- O corpo transacional da função permanece idêntico.
-- =========================================================
