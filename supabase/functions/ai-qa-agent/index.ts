      ...
      const body = new ReadableStream({
        async start(controller) {
          ...
          if (flow === "admissao" || flow === "todos") {
            ...
            steps.push(await runStepStreamed("admissao", "1. Criar Admissão", "INSERT admissoes", async () => {
              ...
              const { data, error } = await supabase.from("admissoes").insert({
                tenant_id: tenantId, nome_completo: "QA Agent — Teste Automatizado",
            ...
            steps.push(await runStepStreamed("admissao", "2. Verificar Admissão", "SELECT admissoes", async () => {
            ...
            steps.push(await runStepStreamed("admissao", "3. Atualizar Status", "UPDATE admissoes", async () => {
              if (!admissaoId) throw new Error("Admissão não encontrada");
              const { error } = await supabase.from("admissoes").update({
                status: "concluido", data_admissao: new Date().toISOString().split("T")[0], salario: 5000,
              }).eq("id", admissaoId);
              if (error) throw new Error(error.message);
              return "Status atualizado para 'concluido'";
            }));

            steps.push(await runStepStreamed("admissao", "4. Verificar Status Final", "SELECT admissoes", async () => {
              if (!admissaoId) throw new Error("Admissão não encontrada");
            ...
            steps.push(await runStepStreamed("admissao", "5. Criar Documento", "INSERT admissao_documentos", async () => {
              if (!admissaoId) throw new Error("Admissão não encontrada");
              const { data, error } = await supabase.from("admissao_documentos").insert({
                tenant_id: tenantId, admissao_id: admissaoId, nome: "RG — Teste QA",
                tipo: "rg", obrigatorio: true, status: "pendente",
              }).select("id").single();
              if (error) throw new Error(error.message);
              return `Documento criado: ${data.id}`;
            }));

            steps.push(await runStepStreamed("admissao", "6. Cleanup", "DELETE", async () => {
              if (admissaoId) {
            ...
            const flowResult: FlowResult = {
              flow: "admissao", flow_label: "Fluxo de Admissão", steps, passed, failed,
            ...
            send("flow_done", flowResult);
          }

          // ═══ FLOW 2: ATESTADO ═══
          if (flow === "atestado" || flow === "todos") {
            send("flow_start", { flow: "atestado", label: "Fluxo de Atestado Médico" });
            const steps: StepResult[] = [];
            ...
            steps.push(await runStepStreamed("atestado", "1. Criar Atestado", "INSERT atestados", async () => {
              ...
              const { data, error } = await supabase.from("atestados").insert({
                tenant_id: tenantId, colaborador_nome: "QA Agent — Colaborador Teste",
                profissional_nome: "Dr. QA Teste", profissional_registro: "CRM-QA-0000",
                tipo: "assistencial", data_emissao: new Date().toISOString().split("T")[0], dias_afastamento: 3,
              }).select("id").single();
              if (error) throw new Error(error.message);
              atestadoId = data.id;
            ...
            steps.push(await runStepStreamed("atestado", "2. Verificar Atestado", "SELECT atestados", async () => {
            ...
            steps.push(await runStepStreamed("atestado", "3. Atualizar CID", "UPDATE atestados", async () => {
              if (!atestadoId) throw new Error("Atestado não encontrado");
              const { error } = await supabase.from("atestados").update({
                cid_codigo: "J11", contem_cid: true, grupo_clinico: "respiratorio",
              }).eq("id", atestadoId);
              if (error) throw new Error(error.message);
            ...
            steps.push(await runStepStreamed("atestado", "4. Cleanup", "DELETE", async () => {
              if (atestadoId) await supabase.from("atestados").delete().eq("id", atestadoId);
              return "Atestado de teste removido";
            }));

            const passed = steps.filter(s => s.status === "success").length;
            const flowResult: FlowResult = {
              flow: "atestado", flow_label: "Fluxo de Atestado Médico", steps, passed,
          ...
          if (flow === "epi" || flow === "todos") {
            send("flow_start", { flow: "epi", label: "Fluxo de EPI (Entrega + Estoque)" });
            const steps: StepResult[] = [];
            ...
            steps.push(await runStepStreamed("epi", "1. Criar EPI", "INSERT epis", async () => {
              ...
              const { data, error } = await supabase.from("epis").insert({
                tenant_id: tenantId, nome: "QA Agent — Luva Teste", ca: "CA-QA-0000",
            ...
            steps.push(await runStepStreamed("epi", "2. Verificar Estoque", "SELECT epis", async () => {
            ...
            steps.push(await runStepStreamed("epi", "3. Registrar Entrega", "INSERT epi_entregas", async () => {
              if (!epiId) throw new Error("EPI não encontrado");
              const { data, error } = await supabase.from("epi_entregas").insert({
                tenant_id: tenantId, epi_id: epiId, colaborador_nome: "QA Agent — Colaborador",
                colaborador_cpf: "999.888.777-00", quantidade: 2,
                data_entrega: new Date().toISOString().split("T")[0], status: "ativa",
              }).select("id").single();
              if (error) throw new Error(error.message);
              return `Entrega registrada: ${data.id}`;
            }));

            steps.push(await runStepStreamed("epi", "4. Verificar Baixa Estoque", "SELECT epis", async () => {
              if (!epiId) throw new Error("EPI não encontrado");
            ...
            const passed = steps.filter(s => s.status === "success").length;
            const flowResult: FlowResult = {
              flow: "epi", flow_label: "Fluxo de EPI (Entrega + Estoque)", steps, passed,
              failed: steps.filter(s => s.status === "fail").length,
              warnings: steps.filter(s => s.status === "warning").length,
              total_duration_ms: steps.reduce((acc, s) => acc + s.duration_ms, 0),
            };
          ...
          if (flow === "plano_acao" || flow === "todos") {
            send("flow_start", { flow: "plano_acao", label: "Fluxo de Plano de Ação" });
            const steps: StepResult[] = [];
            ...
            steps.push(await runStepStreamed("plano_acao", "1. Criar Ação", "INSERT plano_acoes", async () => {
              ...
              const { data, error } = await supabase.from("plano_acoes").insert({
                tenant_id: tenantId, titulo: "QA Agent — Ação de Teste",
                descricao: "Ação criada automaticamente pelo agente de QA",
                status: "pendente", prioridade: "media", responsavel_nome: "QA Agent",
            ...
            steps.push(await runStepStreamed("plano_acao", "2. Criar Tarefa", "INSERT plano_tarefas", async () => {
              if (!acaoId) throw new Error("Ação não criada");
              const { data, error } = await supabase.from("plano_tarefas").insert({
                tenant_id: tenantId, acao_id: acaoId, titulo: "Tarefa de teste QA",
            ...
            steps.push(await runStepStreamed("plano_acao", "3. Concluir Tarefa", "UPDATE plano_tarefas", async () => {
              if (!tarefaId) throw new Error("Tarefa não criada");
              const { error } = await supabase.from("plano_tarefas").update({ status: "concluida" }).eq("id", tarefaId);
              if (error) throw new Error(error.message);
              return "Tarefa marcada como concluída";
            }));

            steps.push(await runStepStreamed("plano_acao", "4. Verificar Trigger", "SELECT plano_acoes", async () => {
            ...
            const passed = steps.filter(s => s.status === "success").length;
            const flowResult: FlowResult = {
              flow: "plano_acao", flow_label: "Fluxo de Plano de Ação", steps, passed,
              failed: steps.filter(s => s.status === "fail").length,
              warnings: steps.filter(s => s.status === "warning").length,
              total_duration_ms: steps.reduce((acc, s) => acc + s.duration_ms, 0),
            };
          ...
          if (flow === "rls_isolamento" || flow === "todos") {
            ...
            steps.push(await runStepStreamed("rls_isolamento", "2. Testar Acesso Anônimo", "SELECT (anon)", async () => {
            ...
            steps.push(await runStepStreamed("rls_isolamento", "3. Verificar Profiles × Tenants", "SELECT", async () => {
            ...
            const passed = steps.filter(s => s.status === "success").length;
            const flowResult: FlowResult = {
              flow: "rls_isolamento", flow_label: "Isolamento Multi-Tenant (RLS)", steps, passed,
              failed: steps.filter(s => s.status === "fail").length,
              warnings: steps.filter(s => s.status === "warning").length,
              total_duration_ms: steps.reduce((acc, s) => acc + s.duration_ms, 0),
            };
          ...
          // ═══ FLOW 6: EDGE FUNCTIONS ═══
          if (flow === "edge_functions" || flow === "todos") {
            send("flow_start", { flow: "edge_functions", label: "Health Check de Edge Functions" });
            const steps: StepResult[] = [];
            const fns = [
            ...
            for (const fn of fns) {
              steps.push(await runStepStreamed("edge_functions", `Ping ${fn}`, "POST (health check)", async () => {
            ...
            const passed = steps.filter(s => s.status === "success").length;
            const flowResult: FlowResult = {
              flow: "edge_functions", flow_label: "Health Check de Edge Functions", steps, passed,
              failed: steps.filter(s => s.status === "fail").length,
              warnings: steps.filter(s => s.status === "warning").length,
              total_duration_ms: steps.reduce((acc, s) => acc + s.duration_ms, 0),
            };
...
Seja direto e técnico. Português do Brasil.`,
    ...
    if (flow === "admissao" || flow === "todos") {
      ...
      steps.push(await runStep("6. Cleanup", "DELETE", async () => {
        if (admissaoId) {
          await supabase.from("admissao_documentos").delete().eq("admissao_id", admissaoId);
          await supabase.from("admissao_historico").delete().eq("admissao_id", admissaoId);
          await supabase.from("admissao_workflow").delete().eq("admissao_id", admissaoId);
      ...
      const passed = steps.filter(s => s.status === "success").length;
      flows.push({
        flow: "admissao", flow_label: "Fluxo de Admissão", steps, passed,
        failed: steps.filter(s => s.status === "fail").length,
        warnings: steps.filter(s => s.status === "warning").length,
        total_duration_ms: steps.reduce((acc, s) => acc + s.duration_ms, 0),
      });
    ...
    // Simplified non-streaming for other flows (kept for backward compat)
    const totalPassed = flows.reduce((a, f) => a + f.passed, 0);
    const totalFailed = flows.reduce((a, f) => a + f.failed, 0);
    const totalSteps = flows.reduce((a, f) => a + f.steps.length, 0);
