...
with chk(ordem, item, esperado, obtido) as (
  select 1, 'planos', '7', (select count(*)::text from public.plans)
  union all
  select 2, 'features', '26', (select count(*)::text from public.features)
  union all
  select 3, 'precos_monthly', '5', (select count(*)::text from public.plan_prices where period='monthly')
  union all
  select 4, 'matriz starter/essential/performance/governanca/enterprise',
         '6/10/15/20/26',
  ...
  union all
  select 5, 'tenants x assinaturas (devem casar)',
         (select count(*)::text from public.tenants),
         (select count(*)::text from public.subscriptions)
  union all
  select 6, 'tenants sem assinatura (deve ser 0)', '0',
         (select count(*)::text from public.tenants t
          where not exists (select 1 from public.subscriptions s where s.tenant_id=t.id))
  union all
  select 7, 'policies de enforcement ligadas (deve ser 0 agora)', '0',
         (select count(*)::text from public.entitlement_gated_tables)
  union all
  select 8, 'funcao tenant_has_feature responde sem erro', 'ok',
         (select case when public.tenant_has_feature(
                   (select tenant_id from public.subscriptions limit 1), 'mod.ponto') is not null
                 then 'ok' else 'ok' end)
)
