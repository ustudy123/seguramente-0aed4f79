-- ============================================================================
-- ENTREGA — espelho e banco de horas passam a falar o mesmo numero
-- Alvo: a apuracao do ponto (funcao nova + um remendo no resumo do espelho)
-- Caso: PONTO-472 (novo)
--
-- O QUE ESTE ARQUIVO CORRIGE
-- Na auditoria de fechamento de 01/09/2026, os dois documentos oficiais da
-- MESMA competencia, emitidos no MESMO dia, para o MESMO colaborador, nao
-- batiam:
--
...
--
-- Sao dois caminhos para o mesmo numero: o espelho RECALCULA na hora, e o
-- relatorio de banco de horas LE a tabela ponto_banco_horas, que e uma
-- fotografia gravada na ultima vez que alguem rodou a apuracao. Entre uma
-- coisa e outra cabe qualquer mudanca — um ajuste aprovado, um atestado, uma
-- marcacao que chegou depois — e nada avisava que a fotografia envelheceu.
--
-- Passa a existir UMA funcao que responde qual e o numero oficial da
-- competencia (public.ponto_banco_horas_oficial), e todo documento le dela:
--   * competencia FECHADA devolve a fotografia gravada (Sumula 338 do TST:
--     competencia fechada nao se recalcula);
--   * competencia ABERTA devolve a apuracao de agora somada aos lancamentos
--     manuais e as compensacoes registradas.
-- Ela ainda informa, por colaborador, o quanto a fotografia esta atrasada.
--
-- O QUE NAO FAZ
-- NAO altera e NAO apaga nenhum saldo: a funcao so LE. A tabela
-- ponto_banco_horas continua sendo gravada pela apuracao, como sempre, e
-- nenhum numero ja gravado e reescrito por este arquivo. As TELAS so passam
-- a usar a fonte nova depois de Publicar no Lovable — este arquivo prepara o
-- banco para isso.
--
...
-- Esperado: as tres linhas com OK.
-- ============================================================================
WITH esp AS MATERIALIZED (
...
SELECT 1 AS ordem,
       'fonte unica do banco de horas'::text AS o_que,
       CASE WHEN to_regprocedure('public.ponto_banco_horas_oficial(uuid, text, uuid, text)') IS NOT NULL
            THEN 'criada' ELSE 'ausente' END AS detalhe,
       CASE WHEN to_regprocedure('public.ponto_banco_horas_oficial(uuid, text, uuid, text)') IS NOT NULL
            THEN 'OK' ELSE 'PENDENTE: a funcao nao foi criada' END AS erro_tecnico
UNION ALL
SELECT 2, 'espelho ligado na fonte unica',
       CASE WHEN (SELECT position('ponto_banco_horas_oficial' IN src) FROM esp) > 0
            THEN 'o saldo do banco no espelho sai da fonte unica'
            ELSE 'o espelho ainda faz a propria conta' END,
       CASE WHEN (SELECT position('ponto_banco_horas_oficial' IN src) FROM esp) > 0
            THEN 'OK'
            ELSE 'PENDENTE: o remendo do espelho nao casou — me envie o pg_get_functiondef(ponto_espelho_resumo)' END
UNION ALL
SELECT 3, 'caso de teste PONTO-472',
       (SELECT count(*)::text FROM public.qa_casos_teste WHERE codigo = 'PONTO-472')
         || ' documentado; '
         || (SELECT count(*)::text FROM public.qa_implementacoes WHERE codigo = 'PONTO-472' AND ativo)
         || ' com rotina ativa',
       CASE WHEN (SELECT count(*) FROM public.qa_casos_teste WHERE codigo = 'PONTO-472') = 1
