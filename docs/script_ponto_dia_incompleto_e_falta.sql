-- ============================================================================
-- ENTREGA — o dia incompleto vira pendencia, e a falta sai do banco de horas
-- Alvo: a apuracao de saldo do ponto (o corpo que estiver vivo nesta base)
-- Casos: PONTO-473 e PONTO-474 (novos)
--
-- DOIS ACHADOS DA AUDITORIA DE FECHAMENTO DE 01/09/2026, os dois cobrando do
-- trabalhador duas vezes pela mesma coisa.
--
-- 1) O DIA COM MARCACAO SEM PAR VIRAVA DEBITO
--    Quando falta uma batida, a apuracao do dia ja marca o dia como
--    "incompleto" e avisa na observacao que as horas do periodo nao pareado
--    NAO foram contabilizadas. A apuracao de saldo ignorava o aviso e fazia a
--    conta assim mesmo: horas contadas menos jornada. Como as horas estao
--    sabidamente incompletas, saia um debito enorme — muitas vezes o dia
--    inteiro — por uma falha de REGISTRO, que e responsabilidade do
--    empregador (CLT art. 74, §2º; Sumula 338 do TST).
--    Agora o dia incompleto vale ZERO no saldo ate alguem ajustar. Credito ja
--    apurado e mantido: o que falta so pode aumentar o tempo trabalhado.
--
-- 2) A FALTA INJUSTIFICADA ERA DESCONTADA DUAS VEZES
--    Ela lancava um debito de uma jornada inteira no BANCO DE HORAS, alem do
--    desconto que ja acontece na FOLHA (o dia e o repouso da semana, Lei
--    605/1949 art. 6). Isso cobra a mesma ausencia duas vezes e ainda apaga
--    hora extra realmente trabalhada em outros dias. O banco de horas existe
--    para COMPENSAR jornada (CLT art. 59, §2º), nao para punir ausencia.
--    Agora a falta nao entra no banco. Ela continua contada e visivel: o
--    espelho tem a coluna de faltas nao justificadas e o total da competencia,
--    que e o que a folha usa.
--
-- EFEITO PRATICO: os dois ajustes mexem no saldo SEMPRE a favor do
-- trabalhador, e apenas em competencias ainda abertas.
--
-- O QUE NAO MUDA: dia justificado, atestado, ferias, feriado, equalizacao,
-- tolerancia, extras e o teto de 120 min/dia seguem exatamente como estao.
-- Nenhum dado e alterado ou apagado — muda a CONTA, refeita a cada consulta.
--
-- Este arquivo e cirurgico: LE o corpo vivo e troca so os dois trechos. Se
-- algum nao casar, NADA e alterado e a conferencia acusa PENDENTE.
-- Idempotente. PRE-REQUISITO: a bancada de testes (tabelas qa_*) instalada.
--
-- ATENCAO: a parte de TELA (o espelho passar a imprimir "Pendencia —
-- marcacao incompleta" no lugar de um zero mudo) so chega com o Publicar no
-- Lovable. Este arquivo cuida da conta.
-- ============================================================================
...
SELECT 1 AS ordem,
       ...
       CASE WHEN NOT faz_conta THEN 'so delega para o miolo (nada a trocar)'
            WHEN position('[dia-incompleto]' IN src) > 0
             AND position('[falta-fora-do-banco]' IN src) > 0
              THEN 'dia incompleto e pendencia; falta fora do banco'
            ELSE 'trechos esperados nao encontrados' END AS detalhe,
       CASE WHEN NOT faz_conta THEN 'OK'
            WHEN position('[dia-incompleto]' IN src) > 0
             AND position('[falta-fora-do-banco]' IN src) > 0
             AND position('v_diff := -v_jornada_efetiva;' IN src) = 0
              THEN 'OK'
...
SELECT 2,
       'casos de teste',
       (SELECT count(*)::text FROM public.qa_casos_teste WHERE codigo IN ('PONTO-473','PONTO-474'))
         || ' de 2 documentados; '
         || (SELECT count(*)::text FROM public.qa_implementacoes
              WHERE codigo IN ('PONTO-473','PONTO-474') AND ativo)
         || ' com rotina ativa',
       CASE WHEN (SELECT count(*) FROM public.qa_casos_teste
                   WHERE codigo IN ('PONTO-473','PONTO-474')) = 2
