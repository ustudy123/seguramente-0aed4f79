-- =====================================================================
-- YourEyes — Camada de perfil completa + rotinas de QA corrigidas (v2)
--
-- COLE ESTE ARQUIVO INTEIRO no SQL Editor do Supabase e execute UMA vez.
-- Pode rodar de novo sem medo: ele é idempotente (refaz o mesmo estado).
--
-- O que a primeira rodada da bateria em produção (08/08) encontrou:
--   · PERFIL-003 falhou: 11 tabelas sensíveis sem a camada de perfil —
--     a base real tem mais tabelas do que o ambiente onde a camada
--     nasceu. A rotina existe exatamente para achar isso.
--   · PERFIL-004 deu erro: a rotina conferia o papel de usuário em
--     usuário na base inteira e estourava o tempo em produção.
--
-- O que este script faz:
--   1) Aplica a camada de perfil nas 4 tabelas (das 11) que guardam
--      dado pessoal identificável:
--        afastamentos_saude, alertas_saude,
--        psicossocial_entrevistas_evidencias, psicossocial_participacoes
--   2) As outras 7 são organizacionais (catálogos de risco, GHE, índice
--      agregado, plano de ação, responsável técnico) e entram como
--      EXCEÇÕES DOCUMENTADAS na rotina PERFIL-003 — cada uma com o
--      porquê escrito no código.
--   3) Reescreve a PERFIL-004 para escolher poucos candidatos por
--      critérios baratos e conferir o papel só nesses — rápida em
--      qualquer tamanho de base. E o erro técnico, se houver, aparece
--      no relatório.
--   4) No final, RODA a bateria de novo e mostra o resultado — a
--      expectativa é: as 5 rotinas com situacao = passou.
-- =====================================================================

SET lock_timeout = '10s';

-- 1) CAMADA NAS QUATRO TABELAS QUE FALTAVAM ----------------------------
-- Blocos independentes: se uma tabela falhar, as outras não são
-- afetadas e o aviso aparece nas mensagens (aba Messages).
DO $rls$
DECLARE
  r RECORD;
BEGIN
...
-- 4) CONFERÊNCIA: RODA A BATERIA DE NOVO -------------------------------
-- Expectativa: as 5 linhas com situacao = passou.
-- Se alguma vier erro, a coluna erro_tecnico diz exatamente o motivo.
SELECT caso,
