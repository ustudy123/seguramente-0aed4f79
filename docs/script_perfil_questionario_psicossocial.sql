-- =====================================================================
-- ENTREGA · Respostas do questionário psicossocial entram na camada de
--           perfil de acesso
--
...
--
-- POR QUE
-- A recomendação nº 1 do parecer de perfis de acesso (07/08) foi
-- implementada em 11 tabelas, mas duas das mais sensíveis do módulo
-- psicossocial ficaram de fora:
--
--   · questionario_psicossocial_respostas — as respostas INDIVIDUAIS do
--     questionário, ligadas ao colaborador;
--   · questionario_psicossocial_convites — nome, CPF, cargo,
--     departamento e o token de resposta de cada convidado.
--
-- Hoje as duas são liberadas por uma regra que só compara o cliente.
-- Qualquer usuário autenticado da empresa — inclusive um colaborador com
-- perfil restrito, que não enxerga o menu de Psicossocial — alcança,
-- pela API, as respostas de saúde mental dos colegas e os tokens dos
-- convites deles. Dado sensível pela LGPD, art. 11.
--
-- E a rotina que deveria vigiar isso não enxergava essas tabelas: ela
-- varre nomes começados por "psicossocial_", e estas começam por
-- "questionario_". O ponto cego caía justamente sobre o dado mais
-- sensível — a vigilância passava dizendo que estava tudo coberto.
--
-- O QUE ESTE SCRIPT FAZ
--   1. Aplica a camada de perfil nas duas tabelas (somente leitura).
--   2. Amplia o alcance da vigilância para o prefixo "questionario_".
--   3. Atualiza a rotina PERFIL-001, que vigiava uma lista fixa de 7
--      tabelas quando a camada já cobre 13.
--
-- O QUE ELE NÃO MEXE
-- Nenhum dado é alterado, criado ou apagado — só regras de leitura.
-- Quem responde o questionário por link/token não é afetado: as regras
-- são restritivas para usuários autenticados, e regras restritivas só
-- valem para os papéis que nomeiam. O acesso anônimo pelo link e a
-- gravação pelas funções do sistema seguem intactos. A escrita não é
-- tocada, porque a exposição é de leitura.
--
...
-- =====================================================================
WITH camada AS MATERIALIZED (
  SELECT
    count(*) FILTER (
      WHERE tablename = 'questionario_psicossocial_respostas'
        AND permissive = 'RESTRICTIVE' AND cmd = 'SELECT')            AS respostas_ok,
    count(*) FILTER (
      WHERE tablename = 'questionario_psicossocial_convites'
        AND permissive = 'RESTRICTIVE' AND cmd = 'SELECT')            AS convites_ok,
    count(DISTINCT tablename)                                          AS tabelas_com_a_camada
  FROM pg_policies
  WHERE schemaname = 'public' AND policyname LIKE 'perfil_restringe_leitura_%'
),
vigilancia AS MATERIALIZED (
  SELECT
    (SELECT r.situacao::text FROM public.qa_executar_descartavel('qa_caso_perfil_001') r) AS perfil_001,
    (SELECT r.situacao::text FROM public.qa_executar_descartavel('qa_caso_perfil_003') r) AS perfil_003
)
SELECT CASE WHEN c.respostas_ok > 0 THEN 'sim' ELSE 'NAO — verificar' END AS respostas_protegidas,
       CASE WHEN c.convites_ok  > 0 THEN 'sim' ELSE 'NAO — verificar' END AS convites_protegidos,
       c.tabelas_com_a_camada                                             AS tabelas_com_a_camada,
       v.perfil_001                                                       AS vigilancia_perfil_001,
       v.perfil_003                                                       AS vigilancia_perfil_003
FROM camada c CROSS JOIN vigilancia v;
