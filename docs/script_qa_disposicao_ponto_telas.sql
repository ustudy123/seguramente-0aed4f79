-- ============================================================================
-- ENTREGA — QA: atualiza a DISPOSIÇÃO dos 6 casos de TELA do Ponto (as-built)
--
-- A disposição desses casos foi gravada em 05/08/2026, ANTES das ondas 2–9, com
-- o motivo genérico "o motor de apuração ainda não existe" aplicado a todos os
-- PONTO-% de uma vez. Hoje o motor existe e o apoio de banco de vários já foi
-- construído. Este script corrige a disposição dos 6 casos e2e (tela) para
-- refletir a auditoria as-built do código, caso a caso.
--
-- NÃO muda rotina de teste nem resultado da bateria — só o texto de disposição
-- do relatório. Idempotente. Roda inteiro em UMA transação.
-- ============================================================================
...
-- Esperado: 6 linhas, uma por caso, com a nova disposição e o motivo começando
-- por "AS-BUILT 2026-08-20". A coluna erro_tecnico deve ser 'OK' nas 6.
-- ---------------------------------------------------------------------------
SELECT
  codigo,
  disposicao,
  left(disposicao_motivo, 32) AS motivo_inicio,
  CASE WHEN disposicao_motivo LIKE 'AS-BUILT 2026-08-20:%'
        AND disposicao IN ('comportamento_correto','aguardando_construcao','decisao_de_produto')
       THEN 'OK' ELSE 'CONFERIR' END AS erro_tecnico
FROM public.qa_casos_teste
WHERE codigo IN ('PONTO-002','PONTO-005','PONTO-006','PONTO-195','PONTO-254','PONTO-363')
ORDER BY codigo;
