-- ============================================================================
-- ENTREGA — QA Ouvidoria: casos complementares (documentacao de testes)
--
-- SO DOCUMENTA. Insere 11 casos de teste na Documentacao de Testes
-- (qa_casos_teste), fechando a cobertura da tela de Ouvidoria. Nao cria
-- tabela, nao altera dado de negocio, nao apaga nada.
--
-- ONDE COLAR
-- No SQL Editor da PRODUCAO (diayjpsrcerycycyaxst), quando aprovado. Mesma
...
-- Termina com uma conferencia: total de casos OUV e quantos desta entrega.
-- ============================================================================
...
BEGIN
  ...
  IF v_mod IS NULL THEN
    RAISE NOTICE 'Modulo pessoas-cultura/ouvidoria nao encontrado — nada inserido.';
    RETURN;
  END IF;
...
SELECT 'Ouvidoria — casos OUV documentados' AS item,
       count(*) AS total,
       count(*) FILTER (WHERE c.codigo IN
         ('OUV-007','OUV-008','OUV-009','OUV-028','OUV-029','OUV-033',
          'OUV-034','OUV-035','OUV-036','OUV-040','OUV-041')) AS novos_desta_entrega
