-- SCRIPT DE ENTREGA — Modernização de 4 rotinas de QA (11/08)
-- Idempotente: pode rodar mais de uma vez. Só recria funções qa_*.
...
-- ─────────────────────────────────────────────────────────
SELECT fn AS funcao,
       CASE WHEN pg_get_functiondef(('public.'||fn||'()')::regprocedure) !~ '999\.[0-9]{3}\.[0-9]{3}-[0-9]{2}'
            THEN 'ok — sem CPF fixo invalido' ELSE 'ERRO — literal antigo ainda presente' END AS situacao,
       NULL::text AS erro_tecnico
FROM unnest(ARRAY['qa_caso_adm_002','qa_caso_colab_033','qa_caso_colab_011','qa_caso_colab_023']) fn;
