-- ============================================================================
-- ENTREGA — a bancada de QA roda em ambiente ANTIGO sem quebrar
--
-- ONDE COLAR: SQL Editor da HOMOLOGACAO (e, quando a trilha for aplicada,
-- tambem na PRODUCAO, depois da parte 16).
--
-- O QUE RESOLVE
-- A bateria do Ponto na homologacao (31/08) voltou com 21 casos em ERRO.
-- O detalhe tecnico mostrou que a causa nao era do sistema testado — era
-- da propria bancada, que pressupunha a estrutura MAIS NOVA do projeto:
--
--   14 casos: "there is no unique or exclusion constraint matching the
--             ON CONFLICT specification". As ferramentas de massa
--             (qa_ponto_dia_min, qa_ponto_dia_horarios) apontavam a chave
--             da apuracao diaria COM empresa, criada pela onda 1. Onde a
--             onda 1 ainda nao chegou, a chave e a antiga e o ON CONFLICT
--             nao encontra indice — a rotina nem comeca. Aqui elas passam
--             a nao depender da forma do indice: atualizam a linha do dia
--             se existir, senao inserem.
--
--   4 casos:  estrutura ausente no destino (coluna nsr; funcao
--             ponto_saldo_dias_competencia_bruto; tabelas
--             feriado_folga_compensatoria e ponto_dossies_fiscalizacao).
--             Eram ERRO, que nada diz a quem le o relatorio. Passam a ser
--             FALHA COM ACHADO, nomeando a peca que falta e o efeito
--             pratico da falta. ERRO e a rotina que nao chegou ao fim;
--             FALHOU e o veredito de que o ambiente nao atende a regra.
--
--   1 caso:   PONTO-354 ja foi tratado em entrega propria
--             (script_ponto_correcoes_bateria_homologacao.sql).
--
--   1 caso:   PONTO-394 continua acusando — e deve. Ele prova que, sem a
--             onda 1, dois vinculos do mesmo trabalhador colidem na chave
--             do dia. E achado legitimo do ambiente, nao defeito do teste.
--
-- GARANTIAS
--   - NAO altera nenhuma regra de negocio. So a bancada que as verifica.
--   - Idempotente: rodar duas vezes nao duplica nem quebra (as sondas so
--     sao envelopadas uma vez).
--   - Nao toca em dado de cliente.
...
-- ---------------------------------------------------------------------------
-- CONFERENCIA FINAL — as pecas desta entrega no ambiente.
-- Esperado: t | t | 4 | OK
-- ---------------------------------------------------------------------------
WITH x AS MATERIALIZED (
  SELECT
    (SELECT prosrc NOT ILIKE '%ON CONFLICT%'
       FROM pg_proc p JOIN pg_namespace n ON n.oid = p.pronamespace
      WHERE n.nspname = 'public' AND p.proname = 'qa_ponto_dia_min') AS massa_min_portavel,
    (SELECT prosrc NOT ILIKE '%ON CONFLICT%'
       FROM pg_proc p JOIN pg_namespace n ON n.oid = p.pronamespace
      WHERE n.nspname = 'public' AND p.proname = 'qa_ponto_dia_horarios') AS massa_horarios_portavel,
    (SELECT count(*) FROM pg_proc p JOIN pg_namespace n ON n.oid = p.pronamespace
      WHERE n.nspname = 'public' AND p.proname IN ('qa_caso_ponto_210_corpo','qa_caso_ponto_301_corpo',
                                                   'qa_caso_ponto_321_corpo','qa_caso_ponto_431_corpo')) AS sondas_com_guarda
)
SELECT massa_min_portavel, massa_horarios_portavel, sondas_com_guarda,
       CASE WHEN massa_min_portavel AND massa_horarios_portavel AND sondas_com_guarda = 4
