-- =========================================================
-- ENTREGA QA — Ponte de cobertura e2e: aprofundamento (16 ligações)
--
-- Cole no SQL Editor da HOMOLOGACAO (projeto fgsblefvdabgdouipigz). É a
-- segunda leva de testes de tela (os que gravam/validam dado) dos 7 módulos
-- novos. Liga cada it() novo ao caso documentado que ele executa.
--
-- Pré-requisito: os 92 casos já documentados (você já rodou
-- docs/script_qa_casos_novos_modulos_leva2.sql). Só INSERE dados em
-- qa_cobertura_e2e (não mexe em estrutura nem na fidelidade). Idempotente:
-- ON CONFLICT (codigo) DO NOTHING. Ao final, conferência SELECT.
-- =========================================================
...
-- =========================================================
-- CONFERÊNCIA — deve mostrar as 16 novas ligações de aprofundamento.
-- =========================================================
SELECT c.codigo, c.spec, c.teste
FROM public.qa_cobertura_e2e c
WHERE c.codigo IN (
  'MURAL-010','MURAL-012','MURAL-021','MURAL-030',
  'BEM-020','BEM-021',
  'FBK-010','FBK-013','FBK-030','FBK-050',
  'TRILHA-010','ASO-012','AVAL-060',
  'CFG-051','CFG-060','CFG-070')
