-- ============================================================================
-- ENTREGA — folga de meio periodo, e o dia curto que nao fecha em silencio
-- Casos: PONTO-476 e PONTO-477 (novos); conserta o PONTO-421
--
-- DE ONDE VEIO
-- Segunda rodada da auditoria de DP. O dia 27/08 de uma colaboradora tinha
-- marcacoes so de manha e fechou como 5h33 de debito, rotulado "Diminui Banco
-- Horas". O dono do produto esclareceu: A TARDE FOI FOLGA. O numero estava
-- certo (jornada 528, trabalhados 195, debito 333); o problema e que ninguem
-- DECLAROU a folga — e o sistema nao tinha como declarar.
--
-- POR QUE ISSO NAO E DETALHE DE TELA
...
--   * ausencia injustificada . desconta dia + DSR na FOLHA (Lei 605/49 art. 6)
-- Sem declaracao, o sistema aplicava sempre a primeira. A Sumula 338 do TST
-- poe o onus da prova no empregador: um espelho que diz "Diminui Banco Horas"
-- sem dizer por que nao sustenta a defesa de que houve folga acordada.
--
-- O QUE ESTE ARQUIVO FAZ
--   1. a folga compensatoria passa a aceitar MINUTOS (meio periodo). Sem
--      minutos, dia inteiro — identico ao comportamento atual;
--   2. o fechamento passa a BLOQUEAR quando ha dia muito abaixo da jornada
--      sem folga, abono ou ajuste declarado. O limite fica em
--      ponto_configuracao.dia_curto_bloqueia_fechamento_minutos (padrao 60);
--   3. conserta, de quebra, um defeito antigo: a rotina de folga pedia a
--      coluna "j.minutos", que nao existe, o erro caia num EXCEPTION e ela
--      usava 480 minutos para todo mundo, em silencio. Numa jornada de 8h48
--      isso debitava 48 minutos A MENOS do banco a cada folga de dia inteiro.
--
-- ATENCAO — MUDA O DIA A DIA DO RH
-- Depois disto, fechamentos que hoje passam PODEM SER BLOQUEADOS ate alguem
-- declarar o motivo dos dias curtos. E o comportamento correto, mas da
-- trabalho na primeira competencia. Para adocao gradual, suba o limite:
--   UPDATE public.ponto_configuracao SET dia_curto_bloqueia_fechamento_minutos = 240;
-- ou desligue por enquanto com 0, e baixe aos poucos.
--
-- NENHUM CALCULO MUDA e nenhum saldo e recalculado. A parte de TELA (o espelho
-- imprimir "Folga compensatoria") chega com o Publicar no Lovable.
--
-- Idempotente. PRE-REQUISITO: bancada de testes (tabelas qa_*) instalada.
-- ============================================================================
...
-- Esperado: as quatro linhas com OK.
-- ============================================================================
SELECT 1 AS ordem, 'folga aceita minutos'::text AS o_que,
       CASE WHEN to_regprocedure('public.ponto_registrar_folga_compensatoria(uuid, text, date, text, integer)') IS NOT NULL
            THEN 'assinatura com p_minutos' ELSE 'ainda so dia inteiro' END AS detalhe,
       CASE WHEN to_regprocedure('public.ponto_registrar_folga_compensatoria(uuid, text, date, text, integer)') IS NOT NULL
            THEN 'OK' ELSE 'PENDENTE: a rotina nao foi substituida' END AS erro_tecnico
UNION ALL
SELECT 2, 'fechamento ve o dia curto',
       CASE WHEN EXISTS (SELECT 1 FROM pg_proc p JOIN pg_namespace n ON n.oid = p.pronamespace
                          WHERE n.nspname = 'public'
                            AND p.proname = 'ponto_fechamento_pendencias_criticas'
                            AND position('[dia-curto-sem-motivo]' IN pg_get_functiondef(p.oid)) > 0)
            THEN 'pendencia critica instalada' ELSE 'trava nao ampliada' END,
       CASE WHEN EXISTS (SELECT 1 FROM pg_proc p JOIN pg_namespace n ON n.oid = p.pronamespace
                          WHERE n.nspname = 'public'
                            AND p.proname = 'ponto_fechamento_pendencias_criticas'
                            AND position('[dia-curto-sem-motivo]' IN pg_get_functiondef(p.oid)) > 0)
            THEN 'OK' ELSE 'PENDENTE: me envie o pg_get_functiondef(ponto_fechamento_pendencias_criticas)' END
UNION ALL
SELECT 3, 'limite configuravel',
       COALESCE((SELECT max(dia_curto_bloqueia_fechamento_minutos)::text FROM public.ponto_configuracao),
                'sem configuracao cadastrada (vale o padrao 60)') || ' min',
       CASE WHEN EXISTS (SELECT 1 FROM information_schema.columns
            ...
            THEN 'OK' ELSE 'PENDENTE: a coluna do limite nao foi criada' END
UNION ALL
SELECT 4, 'casos de teste',
       (SELECT count(*)::text FROM public.qa_casos_teste WHERE codigo IN ('PONTO-476','PONTO-477'))
         || ' de 2 documentados; '
         || (SELECT count(*)::text FROM public.qa_implementacoes
              WHERE codigo IN ('PONTO-476','PONTO-477') AND ativo) || ' com rotina ativa',
       CASE WHEN (SELECT count(*) FROM public.qa_casos_teste
                   WHERE codigo IN ('PONTO-476','PONTO-477')) = 2
