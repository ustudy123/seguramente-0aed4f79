-- =====================================================================
-- Ajuste de ponto pelo app do funcionário — parar de falhar com erro
-- técnico quando a folha do mês está fechada
--
-- Para colar no SQL Editor. Roda inteiro numa transação; rodar duas
-- vezes não muda nada (só substitui as mesmas rotinas de novo).
--
-- O QUE O USUÁRIO RELATOU
-- "Solicitação de Ajuste pelo app do funcionário não está funcionando."
--
-- O QUE ESTAVA ACONTECENDO
-- O pedido saía do celular e voltava a mensagem crua do banco
-- "Período AAAA-MM está fechado. Alterações bloqueadas." — sem dizer o
-- que fazer. Gravar o pedido dispara a consolidação do dia na folha; a
-- consolidação mexe em ponto_diario; e a trava de período fechado só
-- libera quem tem cargo. Pelo app não há usuário logado, então nunca há
-- cargo: o mesmo pedido passa quando o RH faz pela folha interna e
-- explode quando o próprio funcionário faz pelo celular.
--
-- Havia um segundo defeito, maior: o fechamento é de UMA empresa
-- (ponto_fechamentos é único por cliente + empresa + competência), mas a
-- trava olhava só cliente + competência. Fechar a folha da Barros
-- travava o ajuste de todo mundo das outras três empresas do grupo.
--
-- O QUE MUDA
--   1. A trava passa a comparar também a empresa da linha. Fechamento
--      antigo, gravado sem empresa, continua valendo para o cliente
--      inteiro — nada que já esteja fechado se abre.
--   2. As rotinas do app conferem ANTES de gravar e devolvem recado em
--      português, para período fechado e para dia já abonado. Antes, um
--      único dia impedido derrubava o lote inteiro de até 40 itens.
--
-- Não afrouxa nada: continua impossível mexer em folha fechada pelo app.
-- O que muda é que a pessoa passa a saber por quê.
--
-- Só substitui rotinas — nenhuma linha de dado é alterada ou apagada,
-- então não há tabela de resgate a criar. Para voltar atrás, basta
-- reaplicar a versão anterior das três rotinas; a única diferença de
-- comportamento da trava é a linha que compara a empresa
-- (f.empresa_id IS NULL OR f.empresa_id = NEW.empresa_id).
-- =====================================================================
...
-- Esperado: as três linhas com situacao = 'ok'.
-- =====================================================================
WITH def_trava AS MATERIALIZED (
  SELECT COALESCE(pg_get_functiondef(
           to_regprocedure('public.validar_periodo_aberto_ponto_diario()')), '') AS corpo
), def_link AS MATERIALIZED (
  SELECT COALESCE(pg_get_functiondef(to_regprocedure(
           'externo.solicitar_ajustes_ponto_externo_batch(text,jsonb,text,jsonb)')), '') AS corpo
), def_cpf AS MATERIALIZED (
  SELECT COALESCE(pg_get_functiondef(to_regprocedure(
           'externo.solicitar_ajustes_ponto_externo_cpf_batch(text,text,jsonb,text,jsonb)')), '') AS corpo
)
SELECT 'trava de periodo fechado enxerga a empresa' AS item,
       CASE WHEN (SELECT position('f.empresa_id = NEW.empresa_id' in corpo) FROM def_trava) > 0
            THEN 'ok' ELSE 'FALTOU — reaplique o script' END AS situacao
UNION ALL
SELECT 'app (link individual) avisa em portugues',
       CASE WHEN (SELECT position('ja foi fechada pelo RH' in translate(corpo,'á','a')) FROM def_link) > 0
            THEN 'ok' ELSE 'FALTOU — reaplique o script' END
UNION ALL
SELECT 'app (link compartilhado) avisa em portugues',
       CASE WHEN (SELECT position('ja foi fechada pelo RH' in translate(corpo,'á','a')) FROM def_cpf) > 0
            THEN 'ok' ELSE 'FALTOU — reaplique o script' END
ORDER BY 1;
