...
-- das linhas afetadas (backup_links_sem_prazo_AAAAMMDD) e traz, no fim do arquivo, o
-- comando que desfaz.
--
-- AO FINAL
-- Sai UMA conferencia com duas partes: as pecas que chegaram e o VOLUME —
-- quantas linhas das tabelas vivas do Ponto mudaram de quantidade. Nesta parte o volume MUDA — e o que ela veio fazer.
-- ============================================================================
...
DECLARE
  ...
  m text;
BEGIN
  DELETE FROM public.ponto_entrega_volume WHERE parte = 15;
...
-- A parte preenche o prazo de expiracao dos links que estao sem prazo. E alteracao de dado existente — por isso as linhas afetadas sao copiadas antes.
...
-- ---------------------------------------------------------------------
DO $copia$
DECLARE v_nome text := 'backup_links_sem_prazo_' || to_char(CURRENT_DATE, 'YYYYMMDD');
BEGIN
  ...
  END IF;
  EXECUTE format(
    'CREATE TABLE public.%I AS SELECT * FROM public.ponto_links WHERE data_expiracao IS NULL', v_nome);
...
BEGIN
  FOR v IN SELECT tabela FROM public.ponto_entrega_volume
            WHERE parte = 15 AND tabela NOT LIKE '(copia)%'
  LOOP
    ...
    UPDATE public.ponto_entrega_volume
     ...
     WHERE parte = 15 AND tabela = v.tabela;
  END LOOP;
END $volume2$;
...
--   UPDATE public.ponto_links l SET data_expiracao = b.data_expiracao FROM backup_links_sem_prazo_AAAAMMDD b WHERE b.id = l.id;
-- ---------------------------------------------------------------------
