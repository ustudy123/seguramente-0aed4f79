...
-- 1) As funções e as 7 políticas existem? Esperado: 2 funções, 7 linhas.
SELECT to_regprocedure('public.cpf_do_usuario_logado()') IS NOT NULL      AS funcao_cpf_ok,
       to_regprocedure('public.perfil_permite_modulo(uuid, text[])') IS NOT NULL AS funcao_perfil_ok;
...
-- 2) O teste decisivo do parecer, agora executável por SQL:
--    escolha um usuário com perfil restrito e verifique o que ele veria.
--    (No editor auth.uid() é nulo, então rode este pela aplicação — ou
--    simplesmente faça o teste manual do parecer: logar com um usuário
--    de perfil restrito e pedir os atestados pela API. Deve vir só os
--    dele.)
