...
import {
  ...
  Trash2, Pencil, X, FolderOpen, Lock, CheckCircle2,
} from "lucide-react";
...
function TreeNode({
  ...
  const bloqueado = node.status_doc === "bloqueado";
  ...
  return (
    <div>
      <div
        ...
        title={node.motivo_bloqueio || undefined}
      >
        {temFilhos ? (
        ...
        <span className={`truncate flex-1 ${bloqueado ? "line-through decoration-muted-foreground/40" : ""}`}>
          {node.label}
        </span>
        {nivel === 0 && (
          <span className="text-[10px] text-muted-foreground/70 shrink-0" title="Prioridade de documentação">
            P{node.prioridade_doc}
          </span>
        )}
        {bloqueado && <Lock className="w-3 h-3 text-amber-600 shrink-0" />}
        {node.status_doc === "documentado" && <CheckCircle2 className="w-3 h-3 text-emerald-600 shrink-0" />}
        {qtd > 0 && (
            ...
            {!modulo ? (
              ...
              <>
                <div className="flex items-center justify-between mb-4">
                  ...
                  <Button size="sm" onClick={abrirNovo} disabled={modulo.status_doc === "bloqueado"}>
                    <Plus className="w-4 h-4 mr-1" /> Novo caso
                  </Button>
                </div>

                {modulo.status_doc === "bloqueado" && (
                  <div className="flex items-start gap-2 rounded-md border border-amber-200 bg-amber-50 px-3 py-2 mb-4 text-sm">
                    <Lock className="w-4 h-4 text-amber-600 mt-0.5 shrink-0" />
                    <div>
                      <p className="font-medium text-amber-900">Documentação bloqueada neste módulo.</p>
                      <p className="text-amber-800 text-xs mt-0.5">
                        {modulo.motivo_bloqueio ||
                          "As regras de negócio estão em alteração. Documentar agora eternizaria um comportamento que vai mudar."}
                      </p>
                    </div>
                  </div>
                )}
